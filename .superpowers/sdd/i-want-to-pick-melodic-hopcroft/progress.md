...
NOT the boot-blank commit at all. Added FN64_JITTER_PROBE_NOBLANK env to the temporary experiment; it appears nowhere in current code and was removed after measurement. It was used to
...
(2) remove FN64_JITTER_PROBE_NOBLANK scaffolding (it appears nowhere in current code). (3) no further jitter fix.
...
Task 26: complete — rasterizer profiled. CORRECTED framing: the 2 old big terms are ALREADY GONE — unpresented GPU draw_admitted_triangles (~65% of execute) gated off on play lane (1a10b939, production.rs:279, why field dropped 59.84->43.24); SHA-256 "~20%" is STALE (hot guest-read identity migrated to xxh3-128 FastContentDigest, SHA only cold/verify). Current: raster_triangle ~3.88s of ~5.29s execute = 70-75% of execute, ~62-66% of render field, 94-102 ns/covered-pixel. Highest-value candidate = incremental texture S/T/W plane stepping (see #27). Deterministic lever = WM2000 attract pump-census. Finer split proposed a temporary FN64_RASTER_SPLIT probe; it appears nowhere in current code because the experiment was reverted.
...
Task 37 STALLED (agent a795e7e5 set up a monitor and returned without running the capture — 3rd GUI-capture delegation to stall this session). No report, no fresh 480-wide dump, no wm2000 proc. Static analysis exhausted: fill.rs covers() requires x1+1==width (correct IF guest emits x1=479); present framebuffer.rs:80 is a straight per-row copy_width=dst_width.min(src_stride) (copies RDRAM, doesn't invent noise). Whether WM2000's per-frame fill/scissor actually covers col 479 vs leaves it stale is GAME-BEHAVIOR/RUNTIME data — CANNOT be proven statically. Confirming candidate (b) REQUIRES a live 480-wide intro frame dump (cols 476-479, FN64_FRAME_DUMP) — the GUI capture that keeps failing via delegation. => SURFACED TO OWNER: either they run one bounded FN64_FRAME_DUMP=/tmp/wmdump ./scripts/play-wm2000.sh through the intro (I inspect cols 476-479), or I keep retrying the flaky windowed delegation (low confidence). #37 reset to pending pending that.

Task 37 COMPLETE — candidate (b) CONFIRMED (the "stalled" agent had actually finished; capture DID run). 142 live frames, renderer:wgpu, 480x237. Cols 476-478 track scene; COL 479 diverges on all 237 rows with STALE/prior-frame bytes (temporally uncorrelated: white frame -> col 479 dark-red bleed from earlier frame). Root cause: WM2000 per-frame fill covers cols 0-478, VI scans 480; uncovered col 479 = stale RDRAM. Real HW hides via overscan; fn64 presents full active width. Frames: /private/tmp/task37/frames/.
Task 38 dispatched (ae7bc9c9, opus serial sole writer): OWNER CHOSE crop-overscan (hardware-faithful, guest RDRAM untouched). Crop overscan col(s) at present/scanout seam DERIVED from VI geometry (H_START/active-window vs visible area), NOT hardcoded -1. Geometry: vi_scanout.rs ViActiveWindow/output_width:406/derive:600; present framebuffer.rs:80. Gates: live-capture before/after (col 479 fixed + cols 0-478 pixel-identical), unit test on derivation, suite green + gate 33/37, update framebuffer.rs scanout tests coherently.
NOTE: the "monitor and return" pattern is NOT always a stall — task-37 agent returned mid-run text but had actually completed the bounded run; verify by report/artifacts before assuming stall.

Task 38 ESCALATION RESOLVED (owner decision). Agent proved col 479 is genuinely scanned by WM2000's STANDARD NTSC window (H_START 0x006c02ec=640-dot, X_SCALE 0.75, VI_WIDTH 480; both derivation routes give visible_width=480) — so the crop CANNOT be geometry-derived; only analog-TV overscan hides it (a policy constant, not VI data). No oracle can adjudicate: fn64-render-reference is UNVALIDATED (owner flagged), RT64/angrylion are RDP oracles not VI-scanout. => OWNER DECISION: make overscan a PLAYER SETTING with a sensible default + (owner follow-up) a TABBED overlay.
Task 39 dispatched (ac7faf6a, opus serial sole writer): (A) restructure egui overlay (overlay.rs render_over:195/draw_ui, F1, edits InputConfig+TOML) into TABS Input(existing verbatim)/Video(overscan)/Audio(stub — don't invent audio settings). (B) Video overscan setting: present-time edge crop, overscan=0=full scanout(==HEAD purist), default=smallest that hides uncovered col 479 (right-edge 1px for 480-NTSC). Display-crop only, RDRAM untouched. Reuse task-38 staged vi_visible_width plumbing driven by the SETTING. TOML persist + FN64_OVERSCAN env (boot-once) + live slider. Gates: default hides col479 + overscan=0==HEAD byte-identical + kept cols identical (live capture) + tab/crop/default/roundtrip unit tests + suite/gate green.
NOTE: fn64-render-reference is UNVALIDATED (rt64-rust-renderer-unverified-vs-oracle) — never use it as a comparison oracle; only RT64/angrylion are trusted, and neither covers VI scanout.

Task 39 COMPLETE (commit 986f58ad, orchestrator direct — usage-limited so built + verified inline instead of delegating). Tabbed overlay Input/Video/Audio landed. (A) overlay.rs: Tab enum + tab field on Overlay; tab bar (selectable_label, "Input F1"/"Video F2"/"Audio F3" affordance); Input tab = existing bindings+stick UI verbatim (draw_input_tab); Video tab = overscan slider(0..16) + zoom_fill checkbox (draw_video_tab); Audio = stub (draw_audio_tab). render_over now threads &mut VideoConfig + video_dirty->video.save(). demo.rs uses a throwaway non-persist VideoConfig. (B) overscan is a PLAYER SETTING (new module video_config.rs: VideoConfig{overscan,zoom_fill,persist}, ~/.config/fn64/video.toml mirroring InputConfig; default overscan=1 drops exactly the uncovered col; FN64_OVERSCAN env read once in load(); overscan=0=full raw scanout). main.rs present drives target_width from self.video.overscan (crop = stride-overscan, min 1 col), NOT from geometry — the superseded vi_visible_width plumbing (vi.rs/fabric.rs/device_b.rs) was REVERTED (col 479 genuinely scanned; policy not geometry, per task-38 escalation). Kept framebuffer.rs overscan-crop unit test (generic, no geometry claim). F-KEY DESIGN (owner-requested): while overlay OPEN, F1/F2/F3 select Input/Video/Audio (main.rs, checked before global chords, select_tab drops capture); when CLOSED they keep global meanings (F1 toggle / F2 screenshot / F3 HUD). Tests: 92/92 shell pass incl 3 video_config + framebuffer crop. NOTE: game present() path is cfg(fn64_game_linked), only LINKS with a ROM emit — verified type-consistency by inspection (field/call/Tab refs all line up); content-free build compiles + tests green.
Task 40 dispatched (codex-rescue a0cac0e7, base 986f58ad, serial sole writer): make the zoom_fill checkbox WORK. pixels 0.15 hardcodes centered integer-scale letterbox with NO public hook (clip_rect pub(crate)); render_with is the only escape. false=unchanged pixels.render()/render_over letterbox (no-regression critical); true=cached custom fullscreen-blit stretch to fill surface. Shell-only, no RDRAM/raster/render-wgpu. Codex can't run Metal or commit -> writes code, orchestrator verifies on-screen + commits. On-screen verification of BOTH overscan default and zoom-fill still PENDING (needs a live ROM run).

Task 40 COMPLETE (commit 31cca172, codex a0cac0e7 wrote code / orchestrator verified+committed — codex sandbox couldn't commit or run Metal). zoom_fill checkbox now WORKS. New crates/fn64-shell/src/zoom_fill.rs: ZoomFillRenderer = cached fullscreen-triangle wgpu pipeline (pixels::wgpu / wgpu 0.19) sampling the uploaded frame across the whole surface via pixels::render_with, nearest sampler (pixel-art match), ClampToEdge. Pipeline/sampler/layout built once (get_or_insert_with on first zoom-fill present), bind group rebuilt only on Pixels texture resize. main.rs present: else-if self.video.zoom_fill branch AFTER the overlay branch, so zoom-fill applies only when overlay/HUD CLOSED; false branch = untouched pixels.render()/render_over letterbox (byte-identical, no-regression). demo.rs wired with throwaway video config. Cargo.lock re-locked rayon+transitives (crossbeam-deque/epoch/either/rayon-core) — parallel-raster dep catching up, NOT a zoom-fill dep (shell imports no rayon). Verified: type-consistent by inspection (game present() path only LINKS with ROM emit), cargo test -p fn64-shell 92+3 pass. PENDING: on-screen Metal verification of the stretched output + the overscan default hiding col 479 — both need a live ROM run (deferred; usage-limited).
Task 41 dispatched (codex a20ffe47 -> bg codex task task-mt5yafa8-3av2l1): MEASURE-ONLY eval of WM2000 framerate (attract + in-match, ms/DRAWN-frame vs 33.3ms, phase counters not leaf profiles, confirm parallel-raster default delivers projected ~25-27ms live) + audio chop attribution (downstream underrun from slow producer vs independent audio defect, with evidence). Read-only, no tree writes (won't collide). Report -> /Users/jer/.claude/jobs/297000b1/tmp/task-41-report.md. STILL RUNNING.

2026-08-24 task-transport closure: exact-range guest-read payload sharing cut
120-task guest capture 178.1 -> 16.2 ms (97.9% duplicate requested bytes),
and bulk logical copyback cut copyback 127.6 -> 39.0 ms. Task batch, compute,
guest arena, and transactional copyback are production defaults with explicit
`=0` controls. Guest lengths now remain `u32`/typed through ABI transport;
`RdramView::read_logical_bytes` owns the checked host-allocation conversion.
Ten consecutive rebuilt linked rs+wgpu Metal/CoreAudio runs passed: p95
15.447--15.965 ms, max 18.699--19.465 ms, swap-60 hash
3686c6ccce4d3853, swap-120 hash 9f23f803b308b6b4, steady-window audio +0
underruns, zero late callbacks, bounded clean exit. Native CPU/compute exact
ten-repeat test and two-checkpoint ordered-publication test both pass. Clone
census disproved color registry/checkpoint cloning as material perf headroom:
2.556 ms total over 120 tasks (~0.021 ms/task), though broad Clone authority
remains a typing cleanup. `git diff --check` passes. `scripts/lint-docs.py`
still reports its pre-existing 16 errors/3 warnings, including stale generated
docs and old progress env references; none was introduced by this closure.
Broad nextest initially exposed five adapterless raw-DPC failures: the
optional GPU diagnostic path's `create()` check had leaked into the CPU raster
path. Moved GPU extent/pipeline fixture construction behind the diagnostic
enable while retaining renderer-neutral admission checks. Raw-DPC integration
is 51/51; the formerly failing mixed packet passed 10/10 consecutive direct
test-binary runs; render-conformance is 3/3. Combined ABI/render-IR/runtime
nextest is now 843/844, with only the already-reported generated NMR surface
doc drift failing; `c_smoke` passes.
