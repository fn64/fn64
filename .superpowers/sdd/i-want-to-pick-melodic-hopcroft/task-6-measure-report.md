...
- Add three `Instant::now()` brackets inside `execute_raw_dpc_inner`: around the whole call, around the `stage_color_commands` command loop, and around `raster_triangle` itself; accumulate into three atomics reported at census exit (mirror `session_phase_census.rs`'s at-exit reporter). The proposed temporary `FN64_RASTER_SPLIT` env flag appears nowhere in current code because the experiment was never retained. Divide by the combiner census pixel total for ns/pixel. Arm an armed/control pair and quote **shares**, correcting absolutes by the ratio (rule 17).
