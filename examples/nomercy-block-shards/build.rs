...
struct Generation {
    ...
    /// End of the source bytes the SHARDS of this generation span, which may
    /// exceed `source_end` when the last shard is a whole 64 KiB block
    /// overhanging a clamped image (see `resident_shard_counts`).
    ///
    /// This is a separate field rather than `min(count*SHARD, affine_end)`
    /// because that formula is only correct where the natural tiling was
    /// clamped. The BOOT PREFIX ends at the split with a genuinely partial
    /// final shard while its affine extent runs to the end of the boot copy,
    /// so inferring the span there extended the prefix past the split and
    /// broke every prefix shard's identity. `source_end` and this field are
    /// equal for every generation except a clamped resident tail.
    shard_span_end: usize,
    va_start: u32,
}
...
///
/// The prefix tiles `[va_start, split)` and the tail tiles
/// `[split, tail_end)` at 64 KiB with a possibly-partial final shard each.
/// `split` and `tail_end` are ROM offsets inside the boot copy.
///
/// `tail_end` IS NOT UNCONDITIONALLY THE END OF THE BOOT COPY, and assuming it
/// was is what made this generator disagree with `fn64-discover` on the first
/// non-WM2000 title. The resident tail is the part of the boot copy that
/// overlay activations CONTEND for, so it ends where the overlay invalidation
/// union ends -- `fn64-discover/src/generation_topology/mod.rs:448-477` states
/// exactly this and clamps `tail_image_end = load_end.min(invalidation_end)`,
/// calling the unclamped fold "the actual error".
///
/// The end of the boot copy is a HARDWARE CONSTANT (the fixed 1 MiB IPL3 boot
/// DMA), identical for every ROM this path admits, and nothing makes a game's
/// overlays reach it. Measured, both titles:
///
///   title    split       bank end    invalidation union  generator  library
///   WM2000   0x800e1b90  0x80100400  0x80171a60                  2        2
///   Revenge  0x80090000  0x80100400  0x800fafa0                  8        7
///
/// WM2000 agreed only because its union runs PAST the bank end, making the
/// clamp a no-op -- which is why an off-by-one-shard topology bug survived
/// until a second title was generated. The extra shard is real ROM bytes, but
/// they are immutable resident territory no overlay ever writes (measured on
/// Revenge: 20.1% common-MIPS opcodes and 46.0% undefined, against 74-79%/<1%
/// for this ROM's actual code -- it is data), and the library assigns them the
/// same status as the pre-split prefix rather than to the tail generation.
fn resident_shard_counts(split: usize, tail_end: usize) -> (usize, usize) {
    assert!(
        ROM_START < split && split < ROM_START + BOOT_BYTES,
        "resident split {split:#x} must lie strictly inside the boot copy"
    );
    assert!(
        split < tail_end && tail_end <= ROM_START + BOOT_BYTES,
        "resident tail end {tail_end:#x} must lie above the split {split:#x} and inside \
         the boot copy"
    );
    (
        (split - ROM_START).div_ceil(SHARD_BYTES),
        (tail_end - split).div_ceil(SHARD_BYTES),
    )
}
...
/// The stable package-name prefix for THIS TITLE's build: every consumer
...
/// back to.
///
/// This is the *only* place the string is written for this title's own build.
...
///
/// NO MERCY COPY. This file is `examples/revenge-block-shards/build.rs` -- the
/// clamped generator -- with this one constant retargeted. `main()` below
/// constructs the generator via `from_rom_bytes`, which hardcodes this constant
/// as `package_prefix`, so with another title's value a leaf crate named
/// `nomercy-block-shard-00` reaches `package_target("revenge-block",
/// "nomercy-block-shard-00")` and panics "package names either a boot shard or
/// overlay shard". The generator script emits no `build.rs`, so this retarget
/// is the per-title step it leaves to the caller.
///
/// Revenge's copy is the source rather than WM2000's because the resident-tail
/// clamp it carries is a strict generalization: it reduces to the unclamped
/// rule whenever the overlay invalidation union reaches the bank end, which is
/// the case here. MEASURED on this ROM -- split `0x800d9960`, invalidation
/// union `0x8016da30`, well past the boot copy's end -- so the clamp is a
/// no-op and the two generators emit the same 38-package inventory. Copying
/// the clamped version anyway costs nothing and removes a class of future
/// surprise if this title's geometry is ever re-derived.
///
/// ROM: WWF No Mercy (USA), normalized
/// 11640379fdf534b39f34678036ad8e4cdc9b80b4f2cc72411433363372123976.
pub const PACKAGE_PREFIX: &str = "nomercy-block";
...
impl WmShardGenerator {
    ...
    pub fn package_inventory(&mut self) -> Vec<(String, String)> {
        ...
        let tail_end = self.resident_tail_end();
        let (boot_count, tail_count) = resident_shard_counts(split, tail_end);
    ...
    fn resolve_shard(&mut self, package: &str) -> ResolvedShard {
        ...
        // The SHARD COUNT comes from the clamped image; the emitter's
        // containing extent (`image_byte_len` below, which becomes
        // `image_vram_end`) must instead span the WHOLE blocks those shards
        // occupy. The emitter rejects `artifact_end > image_end` as
        // `ShardOutsideImage` (`fn64-recomp-rs-codegen/src/emit/mod.rs:528`),
        // so a final shard allowed to overhang the image must also be allowed
        // to overhang this extent -- otherwise the two rules contradict and the
        // last shard cannot be emitted at all.
        //
        // These are different notions of "image": the runtime generation's
        // identity extent is `[image_start, clamped image_end)` and is what the
        // digest covers, while this one is the artifact envelope the shard's
        // code is bounds-checked against. They coincide for every generation
        // whose clamp is a no-op, which is all of WM2000's and both of
        // Revenge's overlays.
        let shard_count = (generation.source_end - generation.source_start).div_ceil(SHARD_BYTES);
        assert!(
            index < shard_count,
            "shard index {index} is outside generation {} with {shard_count} shards",
            generation.name
        );
        let image_byte_len = generation.shard_span_end - generation.source_start;
        let byte_start = generation.source_start + index * SHARD_BYTES;
        // A shard is a WHOLE 64 KiB block, capped by the affine extent rather
        // than by the generation's image end -- the last one may OVERHANG.
        //
        // `source_end` bounds the shard COUNT (above); it must not also bound
        // the last shard's BYTES. The two are different rules and the runtime
        // states both: the generation's digest covers `[image_start, image_end)`
        // only, so bytes past it inside the final shard are not part of its
        // identity (`fn64-recomp-rs/src/generation/mod.rs:109-126`), while the
        // catalog digest hashes each shard's `start`/`end`, so a truncated
        // final shard disagrees with the whole block the runtime catalog
        // derives (`runtime_generation_catalog.rs:117-126`, whose own comment
        // calls that "exactly the catalog-digest mismatch this produced").
        //
        // For every generation whose `source_end` IS the affine end -- which is
        // every generation on WM2000, and both overlays here -- `min` picks the
        // same value as before and this is a no-op. It differs only for a
        // resident tail clamped below the boot copy's end, i.e. Revenge's.
        let byte_end = (byte_start + SHARD_BYTES).min(generation.shard_span_end);
        let bytes = self
            .rom
            .bytes
    ...
    fn resolve_generation(&mut self, target: PackageTarget) -> (Generation, usize) {
        match target {
            ...
            PackageTarget::Boot(index) => {
                ...
                (
                    Generation {
                        ...
                        // The prefix's final shard is genuinely partial: it
                        // stops at the split, where the tail takes over. No
                        // overhang -- the two runs must not both own a byte.
                        shard_span_end: split,
            ...
            PackageTarget::ResidentTail(index) => {
                let split = self.resident_split();
                // `source_end` is the CLAMPED tail end (see
                // `resident_shard_counts`), so this run tiles exactly the
                // extent `fn64-discover` calls the resident-tail generation
                // and the two produce the same shard count. `affine_source_end`
                // deliberately stays at the end of the whole boot copy: the ROM
                // bytes remain affine with consecutive guest VAs past the clamp,
                // so the final owned word may still take its architectural delay
                // instruction from the following word.
                let source_end = self.resident_tail_end();
                // The tail's shard count comes from the CLAMPED `source_end`,
                // but its shards are whole 64 KiB blocks, so the last one may
                // overhang -- capped at the boot copy's end. This is the one
                // generation where the two ends differ.
                let shard_span_end = (split
                    + (source_end - split).div_ceil(SHARD_BYTES) * SHARD_BYTES)
                    .min(ROM_START + BOOT_BYTES);
                (
                    Generation {
            ...
            PackageTarget::Overlay {
                ...
                (
                    Generation {
                        ...
                        // Overlays truncate their final shard at the text
                        // extent, matching what the pack emits for them.
                        shard_span_end: source_end,
    ...
    fn resident_split(&mut self) -> usize {
        ...
        let tail_end = self.resident_tail_end();
        resident_shard_counts(split, tail_end);
        split
    }

    /// ROM offset where the resident tail stops, i.e. where overlay
    /// activations stop contending for the boot copy.
    ///
    /// Mirrors `fn64-discover`'s `tail_image_end = load_end.min(union_end)`
    /// (`generation_topology/mod.rs:477`). See `resident_shard_counts` for why
    /// this is not unconditionally the end of the boot copy and for the
    /// measured per-title numbers.
    fn resident_tail_end(&mut self) -> usize {
        let va_start = self.boot_va_start;
        let union_end = self
            .overlay_recipes()
            .iter()
        ...
        let bank_end = ROM_START + BOOT_BYTES;
        // `union_end` is a guest VA; convert to the same ROM-offset space the
        // split lives in before clamping. A union below the boot base would
        // underflow, and is impossible here because the split is derived from
        // a `load_start` that is already asserted above `va_start`.
        let union_rom_end = union_end
            .checked_sub(va_start)
            .map(|delta| ROM_START + usize::try_from(delta).expect("tail end fits usize"))
            .expect("overlay invalidation union lies above the boot base");
        union_rom_end.min(bank_end)
    }
...
mod tests {
    ...
    /// `package_inventory`'s call site defaults to `PACKAGE_PREFIX`, so this
    /// tree's build must keep emitting exactly the package-name shapes its
    /// generated leaf manifests carry -- this pins that default, independent
    /// of the round-trip property above. Retargeted from Revenge's copy along
    /// with `PACKAGE_PREFIX` itself; the shapes are identical, only the
    /// prefix differs.
    #[test]
    fn default_prefix_matches_nomercy_package_names() {
        assert_eq!(
            boot_shard_package(PACKAGE_PREFIX, 7),
            "nomercy-block-shard-07"
        );
        assert_eq!(
            resident_tail_shard_package(PACKAGE_PREFIX, 1),
            "nomercy-block-resident-tail-shard-01"
        );
        assert_eq!(
            overlay_shard_package(PACKAGE_PREFIX, 2, 3),
            "nomercy-block-overlay-2-shard-03"
        );
    }
    ...
    fn resident_runs_tile_the_boot_copy_for_every_split() {
        let bank_end = ROM_START + BOOT_BYTES;
        // WM2000: first overlay at VA 0x800E1B90 over a 0x80000400 base, and
        // an invalidation union (0x80171a60) that runs PAST the boot copy, so
        // the clamp is a no-op and the tail end is the bank end.
        assert_eq!(
            resident_shard_counts(ROM_START + 0xe_1790, bank_end),
            (15, 2)
        );
        // No Mercy: first overlay at VA 0x800D9960 over the same base. Under
        // the retired `Boot(0..=13)` / `== 2` constants this was rejected.
        assert_eq!(
            resident_shard_counts(ROM_START + 0xd_9560, bank_end),
            (14, 3)
        );

        // Exhaustive over every word-aligned split AND every shard-boundary
        // tail end at or above it: expanding both runs into their 64 KiB shard
        // extents must reproduce `[ROM_START, tail_end)` exactly -- contiguous
        // from `ROM_START`, no gap, no overlap, no byte past the end -- with
        // only each run's final shard allowed to be partial.
        for split_offset in (4..BOOT_BYTES).step_by(4) {
            let split = ROM_START + split_offset;
            for tail_end in [split + 4, (split + SHARD_BYTES).min(bank_end), bank_end] {
                let (boot, tail) = resident_shard_counts(split, tail_end);
                assert!(boot > 0 && tail > 0, "both runs nonempty at {split:#x}");

                let mut cursor = ROM_START;
                for (run_start, run_end, count) in
                    [(ROM_START, split, boot), (split, tail_end, tail)]
                {
                ...
                assert_eq!(
                    cursor, tail_end,
                    "runs do not cover [ROM_START, tail_end) at {split:#x}/{tail_end:#x}"
                );
            }
        }
    }

    /// The clamp itself, pinned on the two measured titles. This is the
    /// regression that let an off-by-one-shard topology reach a first boot
    /// attempt: WM2000 is insensitive to the clamp and Revenge is not, so a
    /// test using only WM2000's geometry cannot see the rule at all.
    #[test]
    fn the_resident_tail_stops_at_the_invalidation_union() {
        let bank_end = ROM_START + BOOT_BYTES;
        // Revenge (ROM d8c097f8880032fc...): split VA 0x80090000, invalidation
        // union ends at VA 0x800fafa0 -- 21,600 bytes short of the boot copy.
        // Clamped: 7 tail shards, which is what `fn64-discover`'s
        // `build_generation_topology_v1` independently reports for this ROM.
        let revenge_split = ROM_START + 0x8_fc00;
        let revenge_tail_end = ROM_START + 0xfa_ba0;
        assert_eq!(
            resident_shard_counts(revenge_split, revenge_tail_end),
            (9, 7)
        );
        // Unclamped, this is the 8 that disagreed with the library by one shard.
        assert_eq!(resident_shard_counts(revenge_split, bank_end), (9, 8));
        // WM2000's union runs past the boot copy, so clamping cannot change it.
        assert_eq!(
            resident_shard_counts(ROM_START + 0xe_1790, bank_end),
            (15, 2)
        );
    ...
    fn a_split_outside_the_boot_copy_is_rejected() {
        resident_shard_counts(ROM_START + BOOT_BYTES, ROM_START + BOOT_BYTES);
    }

    #[test]
    #[should_panic(expected = "must lie above the split")]
    fn a_tail_end_at_or_below_the_split_is_rejected() {
        let split = ROM_START + 0x8_fc00;
        resident_shard_counts(split, split);
    }
}
