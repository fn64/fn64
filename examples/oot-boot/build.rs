...
//! out-of-tree generated Rust source file and `FN64_BOOT_CONTEXT` to name the
//! black-box `fn64.boot-context.v1` capture for the exact normalized ROM. The
//! source is included as a module and must export this exact, type-checked
//! contract:
//!
//! ```text
...
//! malformed explicit block or boot-context configuration fails loudly; there
//! is no function-lane fallback. The current `recompile_rom` output does not emit
