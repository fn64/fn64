...
///
/// REVENGE COPY. This file is `examples/wm2000-block-shards/build.rs` with
/// this one constant retargeted. `main()` below constructs the generator via
/// `from_rom_bytes`, which hardcodes this constant as `package_prefix` -- so
/// with WM2000's value a leaf crate named `revenge-block-shard-00` reaches
/// `package_target("wm2000-block", "revenge-block-shard-00")` and panics
/// "package names either a boot shard or overlay shard". The generator script
/// emits no `build.rs`, so this retarget is the per-title step it leaves to
/// the caller. ROM: WCW/nWo Revenge Starrcade Edition (USA) v1.01,
/// d8c097f8880032fc63a73a78ad2fcabac8f4b5938b69e615c0689c57c5fdc3c3.
pub const PACKAGE_PREFIX: &str = "revenge-block";
...
mod tests {
    ...
    /// of the round-trip property above. Retargeted from WM2000's copy along
    /// with `PACKAGE_PREFIX` itself; the shapes are identical, only the
    /// prefix differs.
    #[test]
    fn default_prefix_matches_revenge_package_names() {
        assert_eq!(
            boot_shard_package(PACKAGE_PREFIX, 7),
            "revenge-block-shard-07"
        );
        assert_eq!(
            resident_tail_shard_package(PACKAGE_PREFIX, 1),
            "revenge-block-resident-tail-shard-01"
        );
        assert_eq!(
            overlay_shard_package(PACKAGE_PREFIX, 2, 3),
            "revenge-block-overlay-2-shard-03"
        );
    }
