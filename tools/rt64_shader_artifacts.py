...
def parse_dxc_dependencies(depfile: Path, snapshot: Path, expected: dict, denominator: dict) -> dict:
    ...
    for token in tokens:
        ...
        if not path.is_absolute():
            path = snapshot / path
        path = path.resolve()
        relative = relative_to(path, snapshot)
        require(relative is not None, f"DXC dependency escaped the private RT64 snapshot: {path.name}")
...
def produce(args: argparse.Namespace) -> dict:
    ...
    with (
        ...
        for expected in denominator["entries"]:
            source_argument = expected["source"]
            ...
            with tempfile.TemporaryDirectory(prefix=".fn64-dxc-deps-", dir=output) as dependency_temporary:
                depfile = Path(dependency_temporary) / "dependencies.d"
                preprocess_command = [
                    str(compiler), *expected["flags"], "-P", "-Fi", str(preprocessed),
                    "-MD", "-MF", str(depfile), source_argument,
                ]
                preprocess_log = run_dxc(preprocess_command, snapshot)
                active_dependencies = parse_dxc_dependencies(depfile, snapshot, expected, denominator)
            after_dependencies = verify_staged_dependencies(
                snapshot,
                expected["dependency_files"],
                denominator,
                policy["spirv"]["maximum_source_bytes"],
            )
            require(before_dependencies == after_dependencies, f"staged dependency identity changed around preprocessing: {expected['id']}")
            ...
            require(not INCLUDE_RE.search(preprocessed_text), f"preprocessed input retains an include directive: {expected['id']}")
            os.chmod(preprocessed, 0o400)
            compile_command = [
                str(compiler), *expected["flags"], str(preprocessed), "-Fo", str(artifact),
            ]
            ...
            entries.append({
                **expected,
                ...
                "compiler": {
                    ...
                    "artifact_input": expected["preprocessed_artifact"],
                    "preprocess_stdout_sha256": preprocess_log["stdout_sha256"],
                    "preprocess_stderr_sha256": preprocess_log["stderr_sha256"],
                    "compile_stdout_sha256": compile_log["stdout_sha256"],
                    "compile_stderr_sha256": compile_log["stderr_sha256"],
...
def validate_artifact_receipt(
    ...
    for expected, row in zip(denominator["entries"], rows, strict=True):
        require_keys(row, set(expected) | {
            ...
            "compiler_dependency_files", "compiler_dependency_set_sha256",
            "spirv_sha256", "spirv_bytes", "compiler", "wgpu_validation",
        }, f"artifact row {expected['id']}")
        require_keys(row.get("compiler"), {
            "flags", "artifact_input", "preprocess_stdout_sha256", "preprocess_stderr_sha256",
        ...
        require(row.get("compiler", {}).get("flags") == expected["flags"], f"artifact flags changed: {expected['id']}")
        require(row.get("compiler", {}).get("artifact_input") == expected["preprocessed_artifact"], f"artifact was not compiled from retained preprocessed input: {expected['id']}")
        ...
        require({item.get("path") for item in compiler_dependencies} <= set(expected["dependency_files"]), f"compiler observed an undeclared dependency: {expected['id']}")
        require(any(item.get("path") == expected["source"] for item in compiler_dependencies), f"compiler dependency closure omitted source: {expected['id']}")
        for dependency in compiler_dependencies:
            path = dependency.get("path")
            require(path in source_by_path, f"unknown compiler dependency: {expected['id']}")
            require(dependency.get("sha256") == source_by_path[path]["port_sha256"], f"compiler dependency digest changed: {expected['id']} -> {path}")
        for path_key, digest_key, size_key, maximum in (
            ...
            relative = PurePosixPath(expected[path_key])
