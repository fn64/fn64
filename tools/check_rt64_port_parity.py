...
def source_denominator(root: Path) -> tuple[list[tuple[str, str]], dict[tuple[str, str], dict]]:
    ...
    features = load_json(root / "docs/rt64/rt64-public-feature-inventory.json")
...
def rt64_source_id(root: Path) -> str:
    source_id = load_json(root / "docs/rt64/rt64-port-authority.json")["oracle"]["source_id"]
...
def render_doc(rows: list[dict], metadata: dict[tuple[str, str], dict]) -> str:
    ...
    lines = [
        ...
        "`docs/rt64/rt64-port-parity.json`; do not edit this file by hand.", "",
        ...
        "**Status corrected 2026-08-22.** Commits `c8279818`, `c8a6c5e3`, `47e2515a`,",
        "and `744b67bd` revived and gated a separate wgpu-versus-RT64 differential on",
        "Metal. `./scripts/gate-rt64-parity.sh` builds `fn64-render-conformance` with",
        "`--features parity-runner`, submits each corpus command stream to fn64's wgpu",
        "backend and real pinned RT64 C++ through FFI, and byte-compares the results.",
        "Commit `744b67bd` records the last committed gate baseline: 36 total cases,",
        "with **27/32 RT64-authoritative cases byte-identical**, and a passing gate.", "",
        "That differential is **not this ledger's retained runner/verifier protocol** and",
        "closes no manifest row. In particular, 27/32 is not ledger progress:",
        "`REGISTERED_RUNNERS` still contains no wgpu runner; the single closed Rust row",
        "runs the `fn64-render-reference` CPU backend, not wgpu; the existing wgpu runner",
        "is hard-coded to `feature::native-renderer-rdram-sync`; and `ConformanceOutcome`",
        "currently exposes only `target_bytes` and `committed_writes`, not the ledger's",
        "complete frozen observable set. A 2026-08-22 owner-reported source audit sized",
        "closing all pending rows through the retained protocol at **2,000-6,000+ LOC**.",
        "That estimate was not independently reproduced in this update; it is unverified",
        "planning evidence, not implemented or reviewed code.", "",
        "**Live-worktree verification, 2026-08-22 (unverified as a usable differential):**",
        "the required gate was run while six agents had concurrent uncommitted renderer/gate",
        "edits. It exited 1: `RT64 PARITY GATE: FAIL`. The emitted corpus had 35 cases",
        "(33 RT64-authoritative, 2 non-authoritative); the authoritative partition reported",
        "0 byte-identical, 0 differs, 11 one-refused, and 22 both-refused. RT64 refused every",
        "case because SDL video initialization found no display, while current wgpu also",
        "refused many cases for an uncreated backend. The ordinary retained-evidence checker",
        "likewise failed before qualification because RT64 SDL found no display; only its",
        "structural/generated-doc mode was clean. These fail-closed results supersede no",
        "committed baseline and prove no parity; rerun after the concurrent tree settles.", "",
        "Two retained ledger runners remain registered: the RT64 deferred-frame-history",
        "runner and the pure-Rust reference delegate qualifying",
        "`feature::native-renderer-rdram-sync`. That single `RUST_PASS` records only that",
        "fn64's reference CPU rasterizer reproduces an independently derived RDRAM answer",
        "on one fixture at one observable layer. Its RT64 evidence remains `unexercised`,",
        "so it is neither RT64-to-Rust parity nor evidence about the shipping wgpu backend.",
...
def main() -> int:
    ...
    parser.add_argument("--manifest", type=Path, default=Path("docs/rt64/rt64-port-parity.json"))
    parser.add_argument("--doc", type=Path, default=Path("docs/rt64/RT64-PORT-PARITY.md"))
