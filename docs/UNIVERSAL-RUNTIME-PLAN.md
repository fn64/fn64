...
RDRAM DMA writes, status, and DPC ranges before guest execution resumes. DRAM
and XBUS/DMEM DPC ranges both reach the raw renderer seam. Exact known HLE paths
execute rspboot until the first instruction in a DMA-loaded IMEM span, commit
its observable memory/status/entry state, then optimize the ucode phase. Those
HLE calls are host-atomic and do not carry scalar/VU boot registers across the
boundary, so mid-task preemption/resume and exact register continuity are still
open. A family-changing `G_LOAD_UCODE` does not attempt that impossible
mid-HLE transplant: transactional preflight leaves task-entry state untouched,
and the runtime replays the complete ucode phase through LLE, which derives the
real scalar/VU state before the load. HLE selection itself is also exact:
...
submission, including CPU-only DPC work. SP latency is an instruction-count
policy, and DP completion still needs FullSync-derived timing.
...
the dither-filter bit 16; actual analog-output gamma/divot/dither filtering is
still a presentation frontier. Black, public 10-bit fade, and first-line
...
pixel-level GPU capture remains open.
...
for those register and counter semantics.
Cartridge removal changes status and powered data access traps rather than
fabricating a cartridge. The EEPROM deadline is a conservative public-library
compatibility policy rather than measured chip-revision timing. Controller Pak
management-page directory/inode decoding, MBC3 host-off wall time/battery
metadata persistence, and raw Voice commands `0x09`-`0x0D` remain open; raw
Info already shares the high-level initialization readiness state. HLE ucode work
...
loud missing/error gate. Scalar/VU continuity across that optimization boundary,
DP FullSync-derived timing, mid-HLE-task preemption/resume, hardware validation
of VI timing, and save-device timing remain open.
...
accepts direct undereferenced CI8 copy-cycle indices with threshold alpha
compare. It applies the public
...
is exact at the documented blend-color boundary; `G_AC_DITHER` now returns a
named unsupported error because the manual specifies a hardware pseudo-random
threshold, not the former ordered Bayer approximation. `G_SETPRIMDEPTH` plus
...
DPC START/END. All eight raw triangle layouts from SGI *RDP Command Summary*
Tables 11-15 are width-checked and decode signed edge/shade/texture/Z
...
most-significant-bit index. Raw edges now use the public eight-sample
checkerboard coverage mask, with edge and attribute planes evaluated as signed
fixed-point values from Table 12's documented reference points. RGBA16 coverage persists across its visible LSB
...
the same eight sample positions. Silicon-internal accumulator width/truncation
and subpixel correction, high-level coverage-centroid behavior,
