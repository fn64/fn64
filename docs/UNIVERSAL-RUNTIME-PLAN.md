...
| Validation | Unit/oracle tests, C ABI smoke, generated-body inventory/PC-set comparison, trace comparison, audio dumps, and live screenshots exist. Both boot harnesses use guest-quiescence timing. C/rs framebuffer observation matches through 60 swaps but is non-authoritative because the legacy C inventory has 116 callable empty bodies. Prepared native, schema-enabled typed-function, and typed-block lanes retain authoritative reached-destination order. Schema v15 binds those destinations, all five fixed-cycle channels, complete observation geometry, environment, closure, and RT64's nonzero completed workload/present identity; the v3 render envelope binds the same identity with exact post-VI pixels. V3 run-bound journals retain distinct run identities, and verified matrix v11 reconstructs each retained v15 report and requires its declared native/typed-function/typed-block program coverage to equal the observed destination source before accepting its own digest. | No private representative lane has supplied ten semantically identical schema-v15 reports paired with ten distinct v3 journals while exercising every minimum path. Declared run identity is provenance, not proof of an independent process; native-archive identity does not repair the legacy C oracle's missing bodies; DMA and synchronous task evidence do not prove every command semantic; reached destinations do not prove unreachable code; native coroutine continuations remain excluded; focused oracles are not exhaustive; and measured parity only reaches swap 60 with a non-authoritative deeper C oracle. | Regenerate the private module or select a generated block pack; retain ten semantically identical schema-v15 reports at one fixed guest cycle, each paired with a distinct v3 run-bound journal, then populate and verify the exact environment- and program-lane-bound v11 representative matrix. See `RELEASE-GATE.md`. |
