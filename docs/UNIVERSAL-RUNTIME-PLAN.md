...
coverage-gated horizontal divot now preserves full-coverage control rows and
applies the exact componentwise median to otherwise identical non-full rows.
RGBA16 dither restoration and all four AA selectors remain exact pixel-inert
native residuals. Gamma dither uses a deterministic,
retrace-seeded stochastic seven-bit quantizer; the unpublished silicon random
stream is not claimed exact. Black, public 10-bit fade, and first-line
...
physical-console filter traces, MSAA/downsample coverage semantics, and native
implementation of the named residual stages remain open.
