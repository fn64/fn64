...
| Validation | Unit/oracle tests, C ABI smoke, generated-body inventory/PC-set comparison, trace comparison, audio dumps, and live screenshots exist. Both boot harnesses use guest-quiescence timing. C/rs framebuffer observation matches through 60 swaps but is non-authoritative because the legacy C inventory has 116 callable empty bodies. Prepared native, schema-enabled typed-function, and typed-block lanes retain authoritative reached-destination order. Schema v22 binds those destinations, all five fixed-cycle channels, complete observation geometry, environment, closure, RT64's resolved graphics API and TV standard, nonzero completed workload/present identity, normalized ROM identity/class, decoded TV region, exact native Windows workstation build/UBR evidence, the ordered ABI-owned `fn64.rsp-rdp-observations.v2` stream, and DeviceState v9's controller-manager, mapped-fetch/AOT expected-word, and typed pending/completed PFS transaction state. The trusted private-series runner accepts only a policy-revalidated opaque v3 contract, owns exact-ten sequential fresh child launches, and independently recomputes each admitted ROM's normalized header evidence before accepting a report. Admission v7 binds retail/public-homebrew class-specific provenance, a typed build receipt, the exact child/source/recompiled input, and the runner-owned class environment; retained v6 is read-only. Matrix v5 derives report-visible coverage; retained v17 matrices credit fixed NTSC/PAL/MPAL only from header/device/renderer agreement, while report-only verification cannot credit ROM class. The private-series path jointly revalidates an opaque capability's v3 contract, exact-ten receipt, retained reports/journals, raw ROM, runner image, and bound inputs; it exact-matches semantic report and ordered run-event identities before retaining a separately hashed v1 class authority. RT64 target-case credit requires a distinct opaque capability bound to the exact report and ordered matrix events; phase one exposes no production constructor. Missing evidence returns an explicit v6 incomplete assessment rather than a smaller denominator. | Representative private NTSC reference and RT64 LLE/post-VI lanes each supplied ten semantically identical schema-v22 reports with ten distinct terminal v3 journals and were independently reverified locally on 2026-07-22 at fixed cycle `722368695`, with zero unsupported events. A retained public synthetic identified-native XBUS series supplied ten more reports under the same unsupported-instrumentation denominator without receiving private-ROM authority. The current incomplete assessment accepted all three scenarios and all 30 reports, satisfied 12 of 162 requirements, and retained 150 explicit gaps; the other required scenarios remain unsupplied. No positive Windows report or production RT64 platform-case capability has been retained; blocker closure and allowed-source public-microcode identities also remain absent. A class-specific local provenance string is an admitted attestation, not independent public-homebrew provenance. A self-hashed receipt is not transferable process attestation without an external trusted CI/code-signing root; native-archive identity does not repair the legacy C oracle's missing bodies; reached destinations do not prove unreachable code; native coroutine continuations remain excluded; focused oracles are not exhaustive; and measured parity only reaches swap 60 with a non-authoritative deeper C oracle. | Populate a successor allowed-source public-microcode catalog, produce native Windows v22 evidence and production platform-case/blocker authorities, retain external process attestation where required, and retain ten trusted v22 report/journal pairs for each remaining profile scenario. See `RELEASE-GATE.md`. |
...
v22; artifact-identified mapped AOT observations retain their real artifact
and are eligible, while compatibility AOT without one is not. The 64-bit
spaces, XContext, non-kernel privilege behavior, and the legacy whole-function
boundary remain loud rather than borrowing this 32-bit block-lane contract.
...
- Complete remaining CP0, Count/Compare timing, 64-bit TLB translation, non-kernel privilege, delay-slot
...
compatibility `Unspecified` across the wire and native callback: modes 0/1 match an
independent Figure-11 coverage-four oracle, modes 2/3 restore the exact
baseline, and AA-before-divot changes two pixels where either stage alone
changes one. Gamma dither
...
synthetic RGBA16 input, and deliberately generated code 4 with opaque code-7
controls. RT64's
...
code-0/save, untested partial codes 1-3/5-6, and natural/imported hidden
coverage, other
...
code-7 7/8-versus-8/8 alias, untested partial coverage codes, and broader
filter-lattice certification remain open; the bounded code-4 AA-selector stage
