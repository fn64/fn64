...
| Validation | Unit/oracle tests, C ABI smoke, generated-body inventory/PC-set comparison, trace comparison, audio dumps, and live screenshots exist. Both boot harnesses use guest-quiescence timing. C/rs framebuffer observation matches through 60 swaps but is non-authoritative because the legacy C inventory has 116 callable empty bodies. Prepared native, schema-enabled typed-function, and typed-block lanes retain authoritative reached-destination order. Schema v15 binds those destinations, all five fixed-cycle channels, complete observation geometry, environment, closure, and RT64's nonzero completed workload/present identity; the v3 render envelope binds the same identity with exact post-VI pixels. V3 run-bound journals retain distinct run identities. Matrix v5 removes caller-authored coverage, routes evidence by each validated report scenario, derives the v15-visible scenario coverage, and assigns only evidence-backed program/renderer-lane, save, and controller members of the immutable 162-item FullParityV1 profile; absent evidence returns an explicit incomplete assessment rather than a smaller passing matrix. A complete retained result is schema v12. | No private representative lane has supplied ten semantically identical schema-v15 reports paired with ten distinct v3 journals while exercising every minimum path. V15 also does not expose ROM class, decoded TV region, encountered microcode/mechanism evidence, exact Windows/API target, platform case results, or blocker closure, so the current profile remains incomplete by construction. Declared run identity is provenance, not proof of an independent process; native-archive identity does not repair the legacy C oracle's missing bodies; reached destinations do not prove unreachable code; native coroutine continuations remain excluded; focused oracles are not exhaustive; and measured parity only reaches swap 60 with a non-authoritative deeper C oracle. | Add schema-v16 ROM/TV/microcode execution evidence and typed platform/blocker result inputs, regenerate the private module or select a generated block pack, then retain ten report/journal pairs per profile scenario. See `RELEASE-GATE.md`. |
