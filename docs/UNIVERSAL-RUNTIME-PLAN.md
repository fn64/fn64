...
One exact-entry real-ROM diagnostic reached 100,001 instructions in both lanes.
...
ABI-host, and simulation time matched; continuation did not, scheduler steps
were 33,333 versus 25, and the comparison failed. The exact next step is to
expose and diff the pending exit and prepared continuation at publication.
This is one diagnostic only; no fixed or parity result is claimed.
