...
| Validation | Unit/oracle tests, C ABI smoke, generated-body inventory/PC-set comparison, trace comparison, audio dumps, and live screenshots exist. Both boot harnesses use guest-quiescence timing. C/rs framebuffer observation matches through 60 swaps but is non-authoritative because the legacy C inventory has 116 callable empty bodies. Prepared native, schema-enabled typed-function, and typed-block lanes retain authoritative reached-destination order. Schema v17 binds those destinations, all five fixed-cycle channels, complete observation geometry, environment, closure, RT64's nonzero completed workload/present identity, and the ordered ABI-owned `fn64.rsp-rdp-observations.v2` stream. Each recognition event binds the live IMEM text digest, original task microcode-data address/length/digest, and recognized family together. The v3 render envelope binds the same identity with exact post-VI pixels. V3 run-bound journals retain distinct run identities. During an observed invocation the private-series runner owns an OS-random nonce, exact-ten sequential fresh child launches, an exact staged child image, and per-child verification; its file-synced receipt is retained integrity evidence, and its production API accepts only a policy-revalidated opaque v2 contract. Schema-v5 admission requires a typed program-build receipt that recomputes native archives, a typed-observed identity wire, or a typed-block pack and binds the exact child/source/recompiled input. Each production report must match the admitted microcode text and data in one recognized event. Matrix v5 removes caller-authored coverage, routes evidence by each validated report scenario, derives the v17-visible scenario coverage, and assigns evidence-backed program/renderer-lane, save, controller, and RSP/RDP-mechanism members of the immutable 162-item FullParityV1 profile; backend microcode labels remain diagnostic/optimization evidence, while public-microcode credit requires independent exact digest-to-family adjudication by immutable project-owned catalog v1, which is currently empty pending allowed-source provenance; absent evidence returns an explicit incomplete assessment rather than a smaller passing matrix. A complete retained result is schema v13. | No private representative lane has supplied ten semantically identical schema-v17 reports paired with ten distinct v3 journals while exercising every minimum path. V17 still does not expose ROM class, decoded TV region, exact Windows/API target, platform case results, or blocker closure, and empty certified-public-microcode catalog v1 cannot satisfy any of the twelve family requirements, so the current profile remains incomplete by construction. A self-hashed receipt is not transferable process attestation without an external trusted CI/code-signing root; manually declared run identity remains provenance rather than process proof; native-archive identity does not repair the legacy C oracle's missing bodies; reached destinations do not prove unreachable code; native coroutine continuations remain excluded; focused oracles are not exhaustive; and measured parity only reaches swap 60 with a non-authoritative deeper C oracle. | Populate allowed-source identities in a successor certified-public-microcode catalog, add typed ROM/TV evidence, retain external process attestation where transferable provenance is required, add platform/blocker result inputs, regenerate the private module or select a generated block pack, and retain ten trusted report/journal pairs per profile scenario. See `RELEASE-GATE.md`. |
