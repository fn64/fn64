...
| field | current value |
|---|---|
| updated | 2026-08-16 (`WgpuBackend` production triangle-draw integration complete: real triangle-bearing plans now execute and draw through the actual `RenderBackend` seam, end to end) |
...
| last completed result | `FbCommon.hlsli` literal CPU dispatch port (`crates/fn64-render-wgpu/src/fbcommon.rs`, new module; `next-nonoverlap-literal-slice-card.md`), complete for exactly five of the seven remaining unported `FbCommon.hlsli` functions: `uint16_to_float4`, `uint32_to_float4`, `uint_to_float4`, `float4_to_uint8`, `float4_to_uint32` are literal, infallible CPU ports of `UINT16ToFloat4`/`UINT32ToFloat4`/`UINTToFloat4`/`Float4ToUINT8`/`Float4ToUINT32` reusing existing `tmem`/`formats_dither` primitives with no new arithmetic invention. The two remaining functions, `Float4ToUINT16` and `Float4ToUINT`'s RGBA branch, stay open because `rgb_dither::quantize_post_float_rgba16_non_hdr` does not yet accept `Float4ToRGBA16`'s full `float4`/HDR signature -- not attempted in this slice. No WGSL, GPU, production-path, or parity claim of any kind; pure CPU-side dispatch/decode only, unwired (no `pub use` into `lib.rs`, no caller). 30/30 new tests green across 10 consecutive fresh-process runs of `cargo test -p fn64-render-wgpu --lib fbcommon::`, with the two touched source files' SHA-256 hashes frozen and unchanged across all 10 runs. Scoped clippy (`--no-deps --all-targets`, both default and `--features host-gpu-tests`) clean (only expected unwired-`pub fn` dead-code warnings). Explicit-file `rustfmt --check --edition 2021` clean on both touched files. `lint-docs.py` unchanged at the pre-existing 5-error baseline (0 new errors). |
| last completed result (prior) | `WgpuBackend` production triangle-draw integration (`fn64-rt64-wgpu-backend-triangle-integration-card.md`), complete: the actual `RenderBackend` production seam now consumes sealed `Triangle`/`State` commands and drives a real GPU draw -- the gap every other triangle-draw slice in this program left open. `RenderBackend::create` eagerly requests a real device (direct `pollster = "0.3.0"` dependency, already locked transitively, no new resolved dependency) and derives/stores `triangle_target_extent` from its `RenderConfig` atomically alongside the pipeline; a repeated call replaces both together. `PlanCollector` (the sole `ExactRawDpcPlanVisitor` this backend uses) no longer discards `State` or panics on `Triangle`: it tracks `current_other_mode`/`current_combine` in stream order (mirroring `TriangleDrawStateCollector`'s per-triangle-not-whole-plan-final shape, duplicated rather than shared since the sealed `execution_view` API lends a plan to exactly one visitor per call) seeded from `WgpuBackend`'s own durable `rdp_state`, and records each admitted triangle's own command-time snapshot. `stage_and_report` now returns a `StagedOutcome` enum (`TmemLoads`/`TriangleOnly`) instead of a bare tuple: a plan with at least one completed TMEM load always takes `TmemLoads` (`complete_execution`, unchanged), and only a plan with zero completed loads and at least one triangle takes `TriangleOnly` (`RawDpcCoordinator::complete_execution_preserving_physical`, landed `d41fac44`) -- a genuinely mechanical branch, not an inferred one, verified both by a real mixed-plan (load+triangle) test that must still flip the physical slot via the real successor route, and a real triangle-only test that must NOT flip it. Every collected triangle draws in stream order through `TrianglePipelineRenderer::submit_admitted_triangle` with the identity `TriangleRasterParams` derived once from the stored extent; `last_triangle_draw()` updates only after every triangle in a call succeeds, leaving the prior successful value untouched on any failure. |
| next concrete decision | `TextureRectangle`/`TextureRectangleFlip` admission and multi-triangle batching are now landed (`production_adapter.rs` admits both rectangle opcodes as two `RdpTriangleCommand` pushes each with a per-draw viewport override; `draw_admitted_triangles` batches every draw in one `execute_raw_dpc` call into a single `submit_triangles` GPU submission). Remaining: real texture sampling for the rectangle/triangle draw path (this slice's pipeline remains textureless, inherited restriction); connect typed fragment registers, texture-gen output to the combiner/blender/depth inputs for a textured/blended slice; wire a real command-stream dispatcher that calls T1's `push_decoded_raw_dpc` and M4.3.4's `execute_fill_rectangle` from the same walk; continue the typed all-56 wgpu-ingestion assessment. |
| evidence blockers | no matched private-game RT64 performance baseline exists; ShaderNonUniform SPIR-V remains blocked by Naga 30; no presentation/VI/swapchain claim of any kind (execution/readback only, by design); no texture sampling in the triangle draw path. |
