...
| field | current value |
|---|---|
| updated | 2026-08-16 (`WgpuBackend` production triangle-draw integration: eager `create()` triangle-pipeline scaffolding landed; `PlanCollector`/execution-path wiring still blocked on a coordinator prerequisite) |
...
| last completed result | `WgpuBackend` production triangle-draw integration, first slice (`fn64-rt64-wgpu-backend-triangle-integration-card.md` §1a): the actual `RenderBackend` production seam this project's other triangle-draw work does not reach (`production.rs`'s `PlanCollector` still discards `State`/panics on `Triangle`; that gap remains open, see below). `RenderBackend::create` is no longer a no-op -- it blocks once, synchronously, via a new direct `pollster = "0.3.0"` dependency (already locked transitively in this workspace at this exact version via `fn64-shell`'s `pixels`->`wgpu 0.19.4` chain; promoting it to direct added no new resolved dependency, confirmed by a one-line `Cargo.lock` diff), on `UninitializedTrianglePipeline::request()`, storing the resulting `TrianglePipelineRenderer` on a new `WgpuBackend.triangle_pipeline` field. Repeated `create()` calls are an explicit full reset, not a no-op or error. `try_new()` is unchanged -- no GPU involvement outside `create()`, so every existing TMEM-only caller/test is unaffected (verified by a new regression test that never calls `create()` at all). `create()` is a thin wrapper over a new `pub(crate) create_inner`, mirroring this file's own `execute_raw_dpc`/`execute_raw_dpc_inner` split, returning a `pub(crate) WgpuCreateError { NoAdapter, Request }` so this crate's own tests can assert on a genuine `NoAdapter` distinctly from any other failure -- `RenderError::Backend`'s plain `String` reason cannot preserve that distinction once converted. |
| next concrete decision | `WgpuBackend`'s `PlanCollector` must stop discarding `RawDpcSemanticCommandRef::State` and stop treating `Triangle` (landed `2b3ed203`, real now) as `unreachable!()`, snapshotting each triangle's command-time `OtherMode`/`CombineParams` inline (seeded from `WgpuBackend`'s own durable `rdp_state`, mirroring `TriangleDrawStateCollector`'s per-triangle-not-whole-plan-final shape -- `production.rs`'s sealed `execution_view` API lends a plan to exactly one visitor per call, so this is a deliberate duplication of that collector's logic, not a shared call). Execution-path wiring for a triangle-only (zero-TMEM-load) submission is blocked on a coordinator prerequisite: `RawDpcCoordinator::complete_execution_preserving_physical`, dispatched as its own follow-on commit (not part of `2b3ed203`) since `PhysicalTmemState` is non-`Clone` and `complete_execution` has no path to produce a successor for a submission with nothing to load. Once that lands: the triangle-draw call site, extent/raster-params/readback wiring, and a real end-to-end Metal triangle-draw test through the actual `RenderBackend` path. |
| evidence blockers | no matched private-game RT64 performance baseline exists; ShaderNonUniform SPIR-V remains blocked by Naga 30; `RawDpcCoordinator::complete_execution_preserving_physical` has not yet landed, blocking this slice's own triangle-only execution path; production `WgpuBackend` still does not consume `Triangle`/`State` from a sealed plan at all (only admits/composes them outside the `RenderBackend` seam, per `2b3ed203`) |
| verification claim | This slice (eager `create()` scaffolding only) passed `cargo check`/`cargo clippy -p fn64-render-wgpu --no-deps --all-targets` clean under both default and `--features host-gpu-tests`; explicit-file `rustfmt --check --edition 2021` clean; all 13 pre-existing `production.rs` tests pass unchanged; 3 new tests (one default-feature TMEM-only regression check confirming `create()` is never on that path; two `host-gpu-tests` tests -- one asserting the granted adapter's `wgpu::Backend` is genuinely `Metal`, not merely present, the other confirming a repeated `create()` call is a real reset -- both confirmed 10 consecutive clean runs against a real Metal adapter); `lint-docs.py` clean against the pre-existing 5-error `origin/main` baseline, 0 new errors. Production triangle draw (`2b3ed203`) and the two dependencies it built on (`71ab8b4e`, `d6250fcc`) each separately passed their own recorded final-source gates -- see this table's git history for that slice's own verification claim. M2.5.1 has an independently verified 56/56 conditional reference-valid corpus but no runtime/parity claim. |
