...
| last completed result | fragment constant register decode ports RT64's `SetEnvColor`/`SetPrimColor`/`SetBlendColor`/`SetFogColor`/`SetPrimDepth` setters into typed `Color4`/`PrimLod`/`PrimColor`/`PrimDepth` state and matching raw-DPC variants, preserving exact RGBA order, 5-bit `lodMin`, and asymmetric primitive-depth masks/normalization. The immediately preceding slices decode all eight triangle opcodes and reproduce RT64's decoded-triangle-to-three-vertices float conversion, while the combiner now covers the full selector range and exact one-cycle arithmetic in Rust and WGSL. These remain characterization seams: fragment registers are not yet consumed by the combiner/blender/depth path, triangle vertices are not rasterized, and the production adapter rejects these newly decoded command kinds loudly. |
| next concrete decision | finish the literal two-cycle combiner wrap/carry slice; port TextureRectangle/TextureRectangleFlip decode and six-vertex setup; connect typed fragment registers to the combiner/blender/depth inputs; then admit and execute triangle/rectangle draws through the production raw-DPC conveyor. |
| evidence blockers | no matched private-game RT64 performance baseline exists; ShaderNonUniform SPIR-V remains blocked by Naga 30; the one-cycle combiner, fragment registers, RGB dither, and triangle vertices are not yet composed into a production raster draw path; triangle edge/coverage/rasterization and texture-rectangle execution remain absent. |
| verification claim | fragment constant register decode passed `cargo test -p fn64-render-wgpu` 10/10 consecutive clean (597 unit + 13 doctests) plus scoped clippy/rustfmt/lint-docs; triangle vertex conversion and full one-cycle combiner each passed their own final-source 10/10 gates and post-integration full renderer tests; M2.5.1 has an independently verified 56/56 conditional reference-valid corpus but no runtime/parity claim. |
