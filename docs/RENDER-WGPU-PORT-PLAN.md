...
| field | current value |
|---|---|
| updated | 2026-08-16 (unified RDP pure-state admission) |
...
| last completed result | Unified RDP pure-state admission lands as one slice: `TmemStateCommand`/`TmemStateIdentity` (`render_ir.rs`) rename to `RdpStateCommand`/`RdpStateIdentity` in place (all call sites move together, no half-renamed intermediate commit), then gain nine new variants/constructors for the previously-unadmitted pure-state commands -- `SetOtherMode`, `SetColorImage`, `SetFillColor`, `SetEnvColor`, `SetPrimColor`, `SetBlendColor`, `SetFogColor`, `SetPrimDepth`, `SetCombine` -- alongside the four existing TMEM/load variants (`SetTile`/`SetTileSize`/`SetTextureImage`/`SyncLoad`), each with its own `Neutral*` DTO and content-hash identity domain. `production_adapter.rs`'s `push_decoded_raw_dpc` gains nine new push arms (replacing those nine kinds' membership in the prior catch-all rejection arm) plus nine new `StateIdentityTracker` single-slot fields, following the existing `texture_image` single-slot field's precedent, not the 8-slot tile arrays. `raw_dpc/mod.rs`'s `SET_OTHER_MODE` decode arm loses its redundant decode-time `cycle_type() != CycleType::Fill` gate -- confirmed strictly redundant with `plan_fill`'s own independent fill-cycle check, which is untouched and still rejects a non-fill-preceded `FillRectangle` at plan time with its existing reason. All nine RT64 setters (`rt64_rdp.cpp`, pinned commit `5473732a822a4423b5696e7cb18fecc425a59875`) were re-verified directly: none gates on cycle type or RDP mode; `setColorImage`/`setOtherMode`/`setCombine` do value-equality elision only (an optimization, not a gate), which fn64 does not replicate since it already stores unconditionally on every decode. Two independent Fill-cycle-type consumers outside this card's writable paths (`native_contract.rs:775-787`, `targets/fill.rs:335`) were confirmed to re-derive `OtherMode` from their own decoded-command/parameter inputs, not from the removed decode-time gate, so neither depends on it. `raw_dpc/mod.rs`, `production_adapter.rs`, `render_ir.rs`, and `lib.rs` (both crates) are the only touched files; `combiner.rs`, `state.rs`, `fn64-render-reference`, any GPU/WGSL shader, any draw/raster path, and `raw_dpc/triangle*.rs` are untouched, preserving disjointness from the concurrently-landed `rt64-triangle-composition-precursor` slice (`c763253f`/`a3c96c22`). |
| next concrete decision | finish the literal two-cycle combiner wrap/carry slice; connect typed fragment registers, texture-gen output, and `RasterVS` to the combiner/blender/depth inputs and a production draw path; wire a real command-stream dispatcher that calls T1's `push_decoded_raw_dpc` and M4.3.4's `execute_fill_rectangle` from the same walk; port RT64 triangle edge walk and rasterization; continue the typed all-56 wgpu-ingestion assessment |
| evidence blockers | no matched private-game RT64 performance baseline exists; ShaderNonUniform SPIR-V remains blocked by Naga 30; the combiner, fragment registers, RGB dither, fragment PRNG, texture-gen, RasterVS, triangle vertices, and texture-rectangle vertices are not yet composed into a production raster draw path; triangle edge/coverage/rasterization and texture-rectangle execution remain absent; production DPC dispatch still does not consume the owned-read path or call M4.3.4/T1 from a shared per-command walk |
| verification claim | Unified RDP pure-state admission passed its final-source 10/10 clean gate (901 unit + 13 doctests, `fn64-render-wgpu`), scoped clippy (`--no-deps --all-targets`, both crates), explicit-file rustfmt on every touched file, `lint-docs.py` diffed clean against the pre-existing `origin/main` baseline (5 pre-existing errors, no new ones), and one bounded independent adversarial review re-verifying all nine RT64 unconditional-store citations directly against the pinned checkout (all nine confirmed; six of the nine store fully unconditionally with no equality check at all, a strictly weaker gating claim than the card made). `formats_dither` passed its final-source, post-rebase 10/10 gate (828 unit + 13 doctests), scoped clippy/rustfmt/lint-docs (diffed clean against the pre-existing `origin/main` baseline), and one bounded independent adversarial review; `RasterVS` passed its final-source 10/10 gate (690 unit + 13 doctests), scoped clippy/rustfmt/lint-docs, and independent adversarial review; texture-coordinate generation passed its final-source 10/10 gate plus scoped clippy/rustfmt/lint-docs and independent review, then a post-rebase full renderer gate (780 unit + 13 doctests); texture-rectangle decode/vertex conversion, fragment registers, triangle vertices, RGB dither, fragment PRNG, and the one-cycle combiner passed their recorded gates. M2.5.1 has an independently verified 56/56 conditional reference-valid corpus but no runtime/parity claim. |
