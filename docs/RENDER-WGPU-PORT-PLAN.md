...
| last completed result | `raw_dpc` admits the `SetCombine` raw-DPC opcode (`0xfc & 0x3f = 0x3c`, one 64-bit command word) via a new `RawDpcCommandKind::SetCombine(CombineParams)` decode arm that calls the already-ported `combiner::CombineParams::from_wire(w0, w1)` -- previously constructed only by tests -- with `w0` passed through completely unmasked (RT64's `RDP::setCombine` never strips its top opcode byte before storing `combineL`; a dedicated fixture proves byte-for-byte passthrough). `RdpState`/`RdpStateDelta`/`StagedRdpState` gain a `combine: Option<CombineParams>` field following the existing `env_color`/`prim_color` "current value + tracked delta" shape exactly, with a dedicated cross-packet test proving retention across a packet boundary with no intervening `SetCombine`. `production_adapter.rs` gains only the strictly-additive match arms its two exhaustive command-kind matches require to keep compiling; `SetCombine` is rejected there like every other non-TMEM command kind, not newly admitted into the v11 production plan. `endian_swap` ports `FbCommon.hlsli`'s three framebuffer byte-order swap primitives -- `EndianSwapUINT16`, `EndianSwapUINT32`, and `EndianSwapUINT` (`src/shaders/FbCommon.hlsli:9-33`) -- as a pure Rust oracle plus Naga-validated WGSL companion, reusing the already-landed `state::PixelSize` enum for the size dispatch so RT64's own unreachable out-of-range `default` arm has no counterpart to port. The preceding `formats_dither` slice ports `Formats.hlsli`'s three remaining unported primitives -- `FloatToUINT8`, `Float4ToRGBA32`, and `AlphaDitherValue` (`src/shaders/Formats.hlsli:41-54,67,122-127`) -- reusing the already-landed `rgb_dither::dither_pattern_value`/`RgbDither` and `state::AlphaDither` rather than re-transcribing the ordered-dither tables or mode selector a third time. `RasterVS`, texture-coordinate generation, TextureRectangle/TextureRectangleFlip decode and six-vertex conversion, fragment-register decode, the one-cycle combiner, triangle vertex conversion, RGB dither, and fragment PRNG are also integrated as standalone characterization seams. The production adapter still rejects these unadmitted draw commands; none is yet composed into a production raster draw path. |
