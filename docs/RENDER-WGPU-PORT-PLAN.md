...
| last completed result | raw RDP triangle decoded-to-three-vertices conversion (this slice) ports RT64's `decodeTriangles` fixed-point float math against the already-decoded `RawTriangle` payload: the wire YL/YM/YH 16-bit reinterpret (RT64's `(w & 0xFFFF) << 2 >> 2` reduces to a plain 16-bit reinterpret because `DisplayList::w0`/`w1` are `uint32_t`, so both shifts are unsigned/logical and cancel -- not a 14-bit sign extension, a mistake an earlier revision of this slice made and a hostile test now guards against), Q16.16 X/slope conversion, y-floor/intercepts, the three vertex positions in RT64's exact write order, shade/255, perspective and non-perspective texture coordinates and W, and depth scaling, with RT64's exact optional-block-absent zero/W=1 defaults and no defensive fallback on IEEE results (infinities/NaNs preserved). Pure CPU characterization only -- no edge walk, no rasterization, no OtherMode read (caller supplies `texture_perspective`), no draw call. Prior slice (raw RDP triangle command decode) decodes all eight triangle opcodes (`0x08..=0x0f`) into `RawTriangle`; the T1 production adapter still rejects `RawTriangle` exactly like every other still-unadmitted command kind. M4.3.4 executes fill-cycle `FillRectangle` against a real color target. T0 lands the sealed raw-DPC coordinator; T1 integrates a TMEM-only raw-DPC production adapter (`push_decoded_raw_dpc`, still not called by any dispatcher); T3 adds `PendingTmemTransaction::into_physical_successor` |
| next concrete decision | consume the frozen M2.5.1 inventories in a typed all-56 wgpu-ingestion assessment; repair/review M4.2a before parallel LoadTile/LoadBlock execution; wire a real command-stream dispatcher that calls T1's `push_decoded_raw_dpc` and M4.3.4's `execute_fill_rectangle` from the same walk, instead of each being reachable only from its own tests; port RT64 triangle edge walk and rasterization as a separate named slice against the now-decoded vertex payload |
| evidence blockers | no matched private-game RT64 performance baseline exists; the original M2.5 gate is split because required ShaderNonUniform SPIR-V semantics are rejected by Naga 30; no combiner exists in this crate, and triangle handling is command decode plus vertex conversion only (no edge walk, coverage, or rasterization), so texture-rectangle/one-cycle/two-cycle fill and every triangle opcode remain unexecuted; production DPC dispatch still does not consume the new owned-read path or call either M4.3.4's or T1's production entry points from a shared per-command walk |
