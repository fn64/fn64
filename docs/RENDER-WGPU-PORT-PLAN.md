...
| last completed result | `formats_dither` ports `Formats.hlsli`'s three remaining unported primitives -- `FloatToUINT8`, `Float4ToRGBA32`, and `AlphaDitherValue` (`src/shaders/Formats.hlsli:41-54,67,122-127`) -- as a pure Rust oracle plus Naga-validated WGSL companion, reusing the already-landed `rgb_dither::dither_pattern_value`/`RgbDither` and `state::AlphaDither` rather than re-transcribing the ordered-dither tables or mode selector a third time. The preceding `RasterVS` slice ports RT64's screen-scale/offset transform and fixed prim-depth Z override (`src/shaders/RasterVS.hlsl:15-33`). Texture-coordinate generation, TextureRectangle/TextureRectangleFlip decode and six-vertex conversion, fragment-register decode, the one-cycle combiner, triangle vertex conversion, RGB dither, and fragment PRNG are also integrated as standalone characterization seams. The production adapter still rejects these unadmitted draw commands; none is yet composed into a production raster draw path. |
