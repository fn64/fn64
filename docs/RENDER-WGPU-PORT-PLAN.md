...
| field | current value |
|---|---|
| updated | 2026-08-17 (`Math.hlsli` literal CPU dispatch port complete: both `modulo`/`getPerpendicularVector` ported and re-exported from `lib.rs`) |
...
| last completed result | `Math.hlsli` literal CPU dispatch port (`crates/fn64-render-wgpu/src/math_hlsli.rs`, new module; `next-literal-slice-after-depth-card-repair.txt`), complete for both of the file's functions: `modulo` (floored, not truncated, integer modulo matching Python's `%`, with an explicit divide-by-zero passthrough and a stated admitted domain that makes no claim about HLSL signed-overflow wrapping) and `get_perpendicular_vector` (the branchless perpendicular-vector construction, NaN-hostile comparisons preserved literally, `[f32; 3]` as this crate's established untyped vector carrier -- no `glam`/newtype). Both functions are re-exported (`pub use math_hlsli::{get_perpendicular_vector, modulo};`) since the pinned card requires a public API surface for this slice, unlike `fbcommon.rs`/`color_converter.rs`'s unwired-and-unexported precedent. No WGSL, GPU, production-path, or parity claim of any kind; pure CPU-side function port only, unwired (no caller anywhere in the crate). 10/10 new tests (exhaustive small-range sweep, divide-by-zero passthrough, sign-convention differential, `2^24`-boundary float-precision fixture, and a floor-vs-truncation mutation guard for `modulo`; axis-aligned inputs, a perpendicularity dot-product property check, two distinct-NaN-position per-component fixtures, and a comparison/XOR-vs-OR mutation guard for `get_perpendicular_vector`) green across 11 consecutive fresh-process runs of `cargo test -p fn64-render-wgpu --lib math_hlsli::`, with the two touched source files' SHA-256 hashes frozen and unchanged across every run. Full crate suite green under both default (1099/1099) and `--features host-gpu-tests` (1127/1127). Scoped clippy (`--no-deps --all-targets`, both default and `--features host-gpu-tests`) clean for this module. An unrelated `production.rs` clippy error under `--features host-gpu-tests` was present at this slice's original candidate base, `5b3ec87b`, and outside this slice's writable-paths scope; it was independently repaired by the separate `f1a754c0` slice and is now present in this worktree through the required rebase onto current `origin/main`. Explicit-file `rustfmt --check --edition 2021` clean on both touched files. `lint-docs.py` unchanged at the pre-existing 5-error baseline (0 new errors). |
| last completed result (prior) | `FbCommon.hlsli` literal CPU dispatch port (`crates/fn64-render-wgpu/src/fbcommon.rs`, new module; `next-nonoverlap-literal-slice-card.md`), complete for exactly five of the seven remaining unported `FbCommon.hlsli` functions: `uint16_to_float4`, `uint32_to_float4`, `uint_to_float4`, `float4_to_uint8`, `float4_to_uint32` are literal, infallible CPU ports of `UINT16ToFloat4`/`UINT32ToFloat4`/`UINTToFloat4`/`Float4ToUINT8`/`Float4ToUINT32` reusing existing `tmem`/`formats_dither` primitives with no new arithmetic invention. The two remaining functions, `Float4ToUINT16` and `Float4ToUINT`'s RGBA branch, stay open because `rgb_dither::quantize_post_float_rgba16_non_hdr` does not yet accept `Float4ToRGBA16`'s full `float4`/HDR signature -- not attempted in this slice. No WGSL, GPU, production-path, or parity claim of any kind; pure CPU-side dispatch/decode only, unwired (no `pub use` into `lib.rs`, no caller). 30/30 new tests green across 10 consecutive fresh-process runs of `cargo test -p fn64-render-wgpu --lib fbcommon::`, with the two touched source files' SHA-256 hashes frozen and unchanged across all 10 runs. Scoped clippy (`--no-deps --all-targets`, both default and `--features host-gpu-tests`) clean (only expected unwired-`pub fn` dead-code warnings). Explicit-file `rustfmt --check --edition 2021` clean on both touched files. `lint-docs.py` unchanged at the pre-existing 5-error baseline (0 new errors). Note (recorded at the time of this Math.hlsli update, not re-litigated): `ColorConverter` (`rt64_color_converter.cpp` literal CPU dispatch) and `Depth.hlsli` (`depth_encode.rs`, all seven functions) both landed as separate slices after this FbCommon row and before Math.hlsli, but neither updated this capsule's "last completed result" chain at landing time -- their own commits (`55d7e8b0`, `5b3ec87b`) and `git log` remain the authoritative record for those two landings; backfilling their capsule rows is out of this slice's scope. |
| next concrete decision | `TextureRectangle`/`TextureRectangleFlip` admission and multi-triangle batching are landed (`production_adapter.rs` admits both rectangle opcodes as two `RdpTriangleCommand` pushes each with a per-draw viewport override; `draw_admitted_triangles` batches every draw in one `execute_raw_dpc` call into a single `submit_triangles` GPU submission); real committed-TMEM texture sampling in the triangle fragment path is also landed (`9b74f9d7`: RGBA16 committed-TMEM addressing and three-nearest filtering ported into the production triangle fragment shader, gated on effective one-cycle texel dependencies so untextured draws stay valid). `production_adapter.rs`'s `push_decoded_raw_dpc` already dispatches the TMEM/load subset, the nine pure-RDP-state commands, `RawTriangle`, and `TextureRectangle`/`TextureRectangleFlip`; the raw-DPC dispatcher itself is not missing. The narrower remaining gap: `NoOp`, `FillRectangle`, and `FullSync` are still outside `push_decoded_raw_dpc`'s admitted set and are rejected loudly rather than executed -- M4.3.4's `execute_fill_rectangle` is not yet called from this same walk. Also remaining: connect typed fragment registers/texture-gen output to the combiner/blender/depth inputs for a fully wired textured/blended slice beyond triangle-fragment sampling; continue the typed all-56 wgpu-ingestion assessment. |
| evidence blockers | no matched private-game RT64 performance baseline exists; ShaderNonUniform SPIR-V remains blocked by Naga 30; no presentation/VI/swapchain claim of any kind (execution/readback only, by design); `FillRectangle`/`NoOp`/`FullSync` remain outside `push_decoded_raw_dpc`'s admitted command set (rejected loudly, not executed) -- this is not a claim that texture sampling is absent from the triangle path; see `9b74f9d7`. |
