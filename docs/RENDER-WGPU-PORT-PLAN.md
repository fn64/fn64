...
| last completed result | `RasterVS` ports RT64's screen-scale/offset transform and fixed prim-depth Z override (`src/shaders/RasterVS.hlsl:15-33`) as a pure Rust oracle plus Naga-validated WGSL companion. The preceding texture-coordinate-generation slice ports RT64's `normalizeSafe`/`computeTextureGen` (`TextureGen.hlsli`), preserving row-vector matrix order, clamp, linear/non-linear modes, and final UV scale. TextureRectangle/TextureRectangleFlip decode and six-vertex conversion, fragment-register decode, the one-cycle combiner, triangle vertex conversion, RGB dither, and fragment PRNG are also integrated as standalone characterization seams. The production adapter still rejects these unadmitted draw commands; none is yet composed into a production raster draw path. |
