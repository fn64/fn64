...
| field | current value |
|---|---|
| updated | 2026-08-17 (Framebuffer-blend Slice B complete: destination-*color* half of `blend_fragment`'s `Framebuffer` selectors admitted and rendered, extending the already-landed Slice 1 mechanism in place) |
...
| last completed result | Framebuffer-blend Slice B (manual destination-color read, plain `Framebuffer` only) extends the landed Slice 1 mechanism in place -- no second uniform, entry point, or admission predicate. `ResolvedBlendCycle::requires_framebuffer_alpha` narrows `production.rs`'s rejection gate to the `FramebufferAlpha` (coverage-count) case only; a `BlendColorInput::Framebuffer`-selecting cycle is now admitted and threaded onto `TriangleFixture` as `reads_framebuffer_color`. `submit_triangles` gained genuine multi-pass splitting (`split_fixture_runs`, singleton runs for every framebuffer-color-reading fixture, `LoadOp::Load` chaining across runs) with a `padded_bytes_per_row`-stride-correct snapshot buffer at bind group binding 9. `fs_main` gained one conditional branch calling a new, independent full-cycle-loop transcription (`blend_fragment_memory_composite_fn`, `blend_fragment_fn.wgsl`) instead of the admitted-subset `blend_fragment_cycle_fn` -- including the no-`FORCE_BL` last-cycle bypass arm and the separately-derived RGB/alpha final composite the independent review's Bugs A and B required. CPU evidence: 1156/1156 default tests, scoped clippy/rustfmt clean. The required physical-Metal nonzero-row differential test is written (folding in both review-required characterization cases) but could not be run to the 10x bar: this environment has no GPU adapter for any backend, matching every pre-existing `host-gpu-tests` test's identical typed `NoAdapter` failure -- not a regression this slice introduced. `FramebufferAlpha`/coverage-count reads remain rejected; Slice A (native dual-source) remains separately blocked. |
| last completed result (prior) | Production blend wiring Slice 1 is complete for the non-framebuffer selector subset: the real triangle fragment path now evaluates the characterized blender cycles, preserves Copy/Fill and no-blend bypasses, and rejects any `Framebuffer`/`FramebufferAlpha` dependency before GPU submission with `BlendRequiresFramebuffer`. The retained WGSL is differentially checked against `blend_fragment(..., memory: None, ...)`; a corrected host-shareable fixture layout uses offsets 48/80/96 and stride 112. Final-source evidence is 1091/1091 default tests, 1122/1122 Metal tests (3 ignored freeze printers), and 10 consecutive Apple M5 Pro ordinals of the exact WGSL differential, general-divide draw, and pre-submit rejection tests (30/30 fresh processes). This remains a bounded execution/readback result, not framebuffer-dependent blending, presentation, parity, performance, or full-ROM evidence. |
| next concrete decision | Run Slice B's required physical-Metal nonzero-row differential test 10 consecutive clean times on real GPU hardware to close the hardware-verification gap this slice's CPU-only evidence left open. `FramebufferAlpha` remains blocked on a real coverage-count destination, and native dual-source remains separate because wgpu 30 rejects dual-source blending with the current second `R32Uint` status target. Continue the remaining raw-DPC admission gaps (`FillRectangle`, `FullSync`) and the typed all-56 wgpu-ingestion assessment in parallel. |
