...
| last completed result | texture-coordinate generation ports RT64's `normalizeSafe`/`computeTextureGen` (`TextureGen.hlsli`) into a pure `texture_gen` Rust oracle plus a matching Naga-validated WGSL companion, preserving the exact row-vector `mul(lookAt-axis, worldMatrix)` operand order, the unconditional pre-branch `[-1,1]` clamp, both linear (`acos(-x) * 325.94932`) and non-linear (`(x+1) * 512`) modes, and the final `(inputUV / 65536) * texgenUV` scale. The immediately preceding integrated slice is raw RDP TextureRectangle/TextureRectangleFlip decode and six-vertex conversion (`raw_dpc::texture_rectangle`), which ports opcodes `0x24`/`0x25`'s exact 16-byte LLE wire shape (RT64 `texrectLLE`/`texrectFlipLLE`/`DisplayList::p0`/`p1`) and `RDP::drawTexRect`/`drawRect`'s deterministic rectangle-to-six-vertex setup: copy-mode `dsdx >>= 2; lrx \|= 3; lry \|= 3;`, fill-or-copy UL rounding, `FixedRect`'s exact `isNull`/`isEmpty`/`left`/`top`/`right`/`bottom` quarter-pixel rounding (no `movedFromOrigin`/`ExtendedAlignment`/scissor -- an explicit nonclaim), the UV width/height flip swap, `lrs`/`lrt`'s exact `<<7`/multiply/`>>7` order, `vFractionOffset`, and RT64's exact six `triPosFloats`/`triColorFloats`/`triTcFloats` vertex writes; standalone characterization module, not wired into `decode_stream`/`RawDpcCommandKind`/`push_decoded_raw_dpc`. Earlier slices: fragment constant register decode (`SetEnvColor`/`SetPrimColor`/`SetBlendColor`/`SetFogColor`/`SetPrimDepth` into typed `Color4`/`PrimLod`/`PrimColor`/`PrimDepth` state), the full one-cycle combiner (exact selector range and arithmetic in Rust and WGSL), raw RDP triangle decoded-to-three-vertices conversion (RT64's `decodeTriangles` fixed-point float math), and raw RDP triangle command decode (all eight `0x08..=0x0f` opcodes into `RawTriangle`) -- the T1 production adapter still rejects `RawTriangle`, texture-rectangle, texture-gen output, and every fragment-register command kind exactly like every other still-unadmitted command. M4.3.4 executes fill-cycle `FillRectangle` against a real color target. T0 lands the sealed raw-DPC coordinator; T1 integrates a TMEM-only raw-DPC production adapter (`push_decoded_raw_dpc`, still not called by any dispatcher); T3 adds `PendingTmemTransaction::into_physical_successor`. These remain characterization seams: texture-gen output is not yet consumed by any vertex/combiner path, fragment registers are not yet consumed by the combiner/blender/depth path, triangle vertices are not rasterized, and texture-rectangle vertices are not wired into a production draw path. |
| next concrete decision | finish the literal two-cycle combiner wrap/carry slice; connect typed fragment registers and texture-gen output to the combiner/blender/depth inputs; consume the frozen M2.5.1 inventories in a typed all-56 wgpu-ingestion assessment; repair/review M4.2a before parallel LoadTile/LoadBlock execution; wire a real command-stream dispatcher that calls T1's `push_decoded_raw_dpc` and M4.3.4's `execute_fill_rectangle` from the same walk; port RT64 triangle edge walk and rasterization as a separate named slice against the now-decoded vertex payload; wire the now-decoded texture-rectangle six-vertex payload and texture-gen UV output into a production draw path once one exists |
| evidence blockers | no matched private-game RT64 performance baseline exists; ShaderNonUniform SPIR-V remains blocked by Naga 30; the one-cycle combiner, fragment registers, RGB dither, texture-gen, triangle vertices, and texture-rectangle vertices are not yet composed into a production raster draw path; triangle edge/coverage/rasterization and texture-rectangle execution remain absent; production DPC dispatch still does not consume the new owned-read path or call either M4.3.4's or T1's production entry points from a shared per-command walk |
| verification claim | texture-coordinate generation passed `cargo test -p fn64-render-wgpu` 10/10 consecutive clean plus scoped clippy/rustfmt/lint-docs and one independent adversarial review (no findings); texture-rectangle decode/vertex conversion passed `cargo test -p fn64-render-wgpu` post-rebase 10/10 consecutive clean (709 unit + 13 doctests) plus scoped clippy/rustfmt/lint-docs and one bounded independent adversarial review (no behavioral findings) -- see this slice's commit; fragment constant register decode passed `cargo test -p fn64-render-wgpu` 10/10 consecutive clean (597 unit + 13 doctests) plus scoped clippy/rustfmt/lint-docs; triangle vertex conversion and full one-cycle combiner each passed their own final-source 10/10 gates and post-integration full renderer tests; M2.5.1 has an independently verified 56/56 conditional reference-valid corpus but no runtime/parity claim. |
