...
| **B2b** | ~~**Unknown button grammar past the mid-menu plateau.**~~ **Read and measured -- see [`RT64-WM2000-INPUT-GRAMMAR.md`](RT64-WM2000-INPUT-GRAMMAR.md).** The confirm past the plateau is **A**; it moves the guest out of a one-display-list idle state into sustained 3-lists-per-field composition. The scanned-out frame still never changes, which puts the residual gap downstream of input. | **Done.** | That card |
