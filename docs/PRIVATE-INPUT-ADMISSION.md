...
be regenerated under current schema v29. Their private content remains outside the
...
`typed_block_program`, or `identified_native_archive`; the resulting v29
...
cycle, and expected v29 execution source. The executable and receipt must be
...
empty pending allowed-source digest provenance; matrix v19 therefore cannot yet
satisfy any public-microcode requirement. Schema v29 binds RT64's resolved
...
The runner verifies each terminal v3 journal, exact v29
...
microcode-data address and length. Report schema `fn64.release-gate.v29`
...
runner, receipts, reports, and journals. They are historical under v29 and
...
fixture is required for these checks. The no-program path retains no content
...
checked-in `aarch64-apple-darwin` schema-v28 fingerprint. That historical golden includes
...
children) on macOS arm64 on 2026-07-24. It cannot satisfy schema v29 and must
be regenerated. Compiler, SDK, or target drift also fails closed and requires
a separately reviewed target-specific golden. This remains historical public
synthetic mechanism evidence and supplies no private-ROM or representative-ROM
authority.
