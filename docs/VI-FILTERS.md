...
| Active window, X/Y resampling, and field provenance | One typed snapshot retains all fourteen live VI words at each exact retrace deadline. Every field crosses the renderer boundary even when a progressive register image repeats, because field cadence and the retrace-cycle noise seed remain distinct scanout inputs. `H_START` supplies horizontal start/end in output pixels, `V_START` supplies vertical start/end in half-lines, and the post-VI image is allocated at `h_end - h_start` by `(v_end - v_start) / 2`; reversed windows, an active image with zero effective 12-bit source stride, and odd half-line extents trap. A jointly zero H/V window remains a typed inactive live image and never falls back to host geometry. The same snapshot retains source width/origin, 12-bit U2.10 step and subpixel offset for each axis, and progressive/even/odd field identity. A checked host position computes exact `offset + coordinate * step`, retaining all ten fraction bits. Positions at or beyond the last resident reference-framebuffer index become typed `HeldLast`. Vertical interpolation between post-divot rows precedes horizontal interpolation; mode 3 uses the same coordinate generators with lower-sample replication. Gamma follows. Blank, fade, and repeat-line retain the programmed active extent. Backend-only callers without live registers retain explicit identity compatibility state. RT64 consumes the live origin and effective source stride, applies its one-row or odd-serrated two-row origin bias, and retains the complete image across later task/resize refreshes. The deterministic reference path retains origin/stride but still samples its resident RDP framebuffer rather than re-reading arbitrary VI origin/stride bytes from RDRAM. | Active-window field units, the linear topology, register fields, ordering, and interlaced AA row spacing follow public interfaces and patents. The accumulated host integer width is deliberately unnamed. Ten-bit positive sums round to nearest and `HeldLast` is fn64's explicit high-border policy. Exact silicon edge inclusion, accumulator/tie behavior, border fetch, odd-half-line behavior, field phase, and arbitrary reference-path origin/stride source selection remain open; fn64 rejects rather than invents the odd-half-line case. Field-specific raw Y offsets are honored, but no additional phase is inferred from parity. |
...
and blank/fade/repeat geometry. Shared renderer and ABI tests retain the exact
fourteen-word image, raw-MMIO authority, and multi-field deadline stepping; the
RT64 adapter test crosses the Rust/C/C++ boundary and proves failure-checked
no-argument retained-state reapplication.
...
- Exact resampling accumulator precision/rounding, border fetch, active-window
  edge inclusion, odd-half-line behavior, any phase beyond the field-specific
  raw Y offset, and field/AA-mode fetch timing.
- Reference-path source selection from arbitrary live VI origin/stride RDRAM;
  RT64 consumes those fields, while the deterministic reference path currently
  filters its already-resident RDP framebuffer.
