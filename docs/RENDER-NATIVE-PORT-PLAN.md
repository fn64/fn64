# fn64-render-native: porting RT64 to Rust (done better)

The plan for `NativeBackend` — fn64's pure-Rust, wasm-capable RDP renderer,
...
Crate name: **`fn64-render-native`** (parallel to `fn64-render-rt64`, exactly
as `fn64-recomp-native` parallels the N64Recomp adapter).

See `RT64-GAP-REGISTER.md` for the cited gap list and `RENDER-BACKEND-PLAN.md`
for the wrap-then-port decision.

## Guiding principle: oracle-faithful BEHAVIOR, idiomatic Rust STRUCTURE
Same model that made `fn64-recomp-native` work: the *behavior* is bit-exact
...
2. **`hle/` + `rdp`/`rsp`/`tmem` (the RDP pixel pipeline) — PORT the algorithm,
   Rust-native memory/error model.** The crown jewels (framebuffer reinterpret,
...
module: render the same DL through both `Rt64Backend` and `fn64-render-native`,
...
(the wgpu boundary). Grow `fn64-render-native` module by module, each gated.
