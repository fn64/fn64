...
`fn64.release-gate.v27` schema, cycle, exact ordered channel set, byte lengths,
and channel hashes. Schema v27 emits each closure path's typed observation
...
For ROM input, v27 additionally binds the declared class, source z64/n64/v64
...
credit. On Windows, v27 also requires an exact native workstation identity:
...
rather than reinterpreted under the v27 device-authority wire.
The Memory
...
alone does not bind the scenario's private-input or environment (although v27's
...
schema. Moving from v26 to v27 intentionally changes both the artifact root
and `report_sha256`: v27 requires the expanded internal DeviceState v14 wire,
which binds the process-monotonic admission generation of every loaded,
running, yielded, or in-flight RSP task owner. A retained v26 report cannot be
accepted under the new incarnation-safe owner semantics. V26 introduced
DeviceState v13's complete ABI-owned RSP interpreter continuation described
below. The earlier v24-to-v25 transition added DeviceState v12's DPC
CLOCK, BUFBUSY, PIPEBUSY, and TMEM projection. V24 added DeviceState
v11's frozen audio-task execution policy and translated artifact identity. V23 added DeviceState v10's AI
and DPC latches and pending transactions. V22 added DeviceState v9's executor, HostState, mapped-fetch/AOT,
and SI/PFS transaction projection. V21 made memory/audio/trace observations boundary-owned,
...
root can be relabeled as v27 evidence.
...
pre-v27 artifact root and contradictory closure states: unexercised means zero
...
Schema v27 retains `execution.unsupported-event-source` as a twelfth mandatory
path and the host-owned machine-readable observation geometry introduced by
v6. Its internal `fn64.device-evidence.v14` channel retains schema v7's compact guest register projection,
...
metadata. DeviceState v10 additionally binds every guest-visible AI latch,
every DPC START/END/CURRENT/STATUS register, and the tokenized pending DPC
range and RDRAM-or-DMEM source. DeviceState v11 added the install-once
audio-task execution policy and, for translated diagnostic mode, the exact
host-artifact SHA-256. DeviceState v12 additionally binds DPC CLOCK, BUFBUSY,
PIPEBUSY, and TMEM. DeviceState v13 added the ABI-owned RSP interpreter state
with distinct `Reset`, `Exact`, `HleCompatibility`,
`HleCompatibilityUnavailable`, and `InFlight` tags. Exact and compatibility
states bind all 32 scalar registers, SP DMA addresses and lengths,
jump/resume state, all 32 eight-lane vector registers, every signed 48-bit
accumulator lane, VCO/VCC/VCE, divider input/valid/output latches, SP status
and semaphore, all eight DPC registers, and every ordered queued DPC
...
the latter two bind both task offset and generation. Report construction freezes the same policy
at the committed VI edge and accepts only `LleAccuracy`; unconfigured,
translated, and diagnostic-skip execution cannot satisfy a fixed-cycle release
claim. The DeviceState v7 wire additionally binds the future-affecting raw
...
phase. V14 additionally binds each token and lineage generation plus the next
generation that a successful `osSpTaskLoad` will mint. Host pointer values are excluded while
...
callable pointers remain excluded. Internal DeviceState v8 added the ABI-owned
Controller Manager state plus mapped-fetch physical-bank and mapped-AOT
identity. Internal v9 additionally binds every mapped-AOT expected word and
the complete typed pending or posted-before-resume `osPfsIsPlug` transaction,
...
future device result; their typed observations instead enter the canonical v27
...
boundary fail closed. The v27 RSP/RDP stream is likewise append-only release
...
v27 report, private admission, and paired verifier consume that stream as the
...
its host cannot mark them covered by declaration, and the schema-v27 report
...
identity. A closed v27 report is release evidence only when paired with that
...
independent of the schema-v27 wire shape and its geometry-bound encoding.
...
Schema v27 binds both paths through one typed descriptor. Reference capture can
...
Build evidence does not stand in for runtime microcode kickoff identity. Each v27
...
Retained v27 validation requires every task address to name a complete 64-byte
...
but require schema-v27 regeneration and do not close the remaining
...
`x86_64-pc-windows-msvc`. Native Windows execution and positive Windows v27
...
report emitted before the run. The checker compares it with every v27
...
AI work, executes a registered synthetic audio ucode through Load+StartGo,
blocks until its SP completion, then loads and executes the graphics task. A
load replaced before StartGo cannot satisfy either task-execution closure path.
The scenario also performs a positive standard-controller read through the
public sequence: one-time Controller Manager initialization, timed read start
on a dedicated queue, blocking SI-completion receive, then data retrieval. Its
direct-IMEM synthetic RSP program starts a
40-byte RDRAM-to-DMEM DMA and polls `DMA_BUSY` with the
...
Figure 4-3 wait loop before enabling XBUS through the Table 4-5 DPC status
command. It then writes DPC START/END, reaches BREAK, and forwards the captured
...
edge, and captures all five schema-v27 channels plus the ABI-owned RSP/RDP
stream.
...
each archive, registers the linked C entry as a generated destination, and
actually enters it through the ABI-owned native-entry hook before committing
...
The historical schema-v22 XBUS extension passed ten independent feature-enabled
processes on 2026-07-22 at guest cycle `1562500`. All ten reports had semantic
SHA-256 `5fcb34d990ed0e17939028cdbe048ae5888084cf8c8d74c78aa20a95bd1000b4`
and artifact-root SHA-256
`333dc943a42af3e8e1115160201198e12bcc6dc34c9ef712ee63d9c0f1bfc77f`;
the paired series verifier accepted all ten distinct v3 journals. After the
DMA-busy wait and valid Controller Manager call sequence landed, the
feature-gated integration test passed ten consecutive parent invocations,
each launching and verifying its own ten fresh children (100 children total,
not one 100-event series). It requires zero unsupported events and the exact
40-byte XBUS observation in every report, then feeds each series to matrix v5.
...
private-ROM authority and no trusted-runner receipt. Caller-supplied run-event
identities retain series integrity but are not process attestation.
...
one exact schema-v27 `report_scenario`, private-input SHA-256, report SHA-256,
...
coarse platform coverage cannot manufacture that credit. A Windows v27 report
...
exactly ten schema-v27 reports, each paired with its terminal v3 journal and a
...
Schema v27 exposes normalized ROM identity, a host-supplied typed ROM class,
...
report both fail. The child's schema-v27 report template makes this authority-
...
exact v27 report scenario and semantic report SHA, exact ordered matrix run
...
Schema v27 still cannot expose blocker closure. Valid v27 evidence
...
matrix, that report must name the exact current host identity (including the
complete native Windows-version evidence), matching API, and pinned RT64
source, and its adapter identity must equal the current case-feature source
digest. Successful preflight produces an opaque ticket binding the target,
case, host, retained-series identity, canonical pinned RT64 tree, repository
workspace, and adapter source digest. The native runner consumes that ticket
and rechecks its mutable identities immediately before the build and again
after the build before the first case child launches; a source or retained-
series change while an earlier requested case runs therefore cannot make a
later case spend its native repeat bar. The RT64 directory must be an absolute
canonical path to the clean pinned source.
...
recomputes every declaration SHA, reconstructs each retained v27 report and
...
platform-case authorities. V16 lacks v27's boundary-owned observations and
...
credit inside the previous canonical incomplete assessment. Schema v27 now
requires regeneration before any of them are current. The exact state is:

| ROM/report class | Mechanism available | Certified evidence retained |
| --- | --- | --- |
| Synthetic fixtures and end-to-end runner | Five-channel fixed-cycle reports, v3 report/journal/run-event binding, real executor/device/RSP/RDP/VI/reference-render boundaries, derived matrix coverage, and the canonical incomplete assessment are available. | Ten retained public identified-native XBUS v22 reports historically supplied generic native/reference, no-cartridge-save, standard-controller, DRAM-DPC, and XBUS-DPC mechanism rows. They have no private-ROM authority or trusted private-series receipt, and require v27 regeneration; an incomplete assessment is not a complete retained matrix. |
| OoT NTSC 1.0, Rust lane, reference LLE | Private host wiring, committed-VI capture, complete-RDRAM observation, an explicit source-hash-bound `BlockProgram` host-selection seam, an artifact/schema-bound whole-function entry stream, a create-new receipt/source-wire producer, runner-staged exact ROM/microcode-pair admission, and same-event kickoff check exist. The v27 report additionally owns memory/audio/trace bytes at the boundary, binds the unsupported-instrumentation denominator, and requires DeviceState v14. | Ten fresh schema-v22 processes completed and reverified on 2026-07-22 at cycle `722368695`; the retained private receipt binds their exact semantic report and ten run identities, but they are historical under v27. |
...
`retail_cartridge`, `ntsc`, `native_archive/reference_lle_accuracy`,
...
series bind the same compiled unsupported-instrumentation denominator; no v27
series or assessment has yet been retained. The
...
- Schema v27 aggregates the modeled device fabric, executor control,
  ...
  v27 digests do not assert byte-equivalent native coroutine stack frames;
...
DeviceState v14 proves equality only for the modeled single-owner state. It
does not close measured AI/RDP timing or counters, DPC FREEZE/FLUSH behavior,
subword RCP accesses, native-C observation while a task is in flight, or
silicon bus/cache details. Those residuals remain explicit even when every v27
