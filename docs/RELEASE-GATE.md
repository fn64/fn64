...
`fn64.release-gate.v30` schema, cycle, exact ordered channel set, byte lengths,
and channel hashes. Schema v30 emits each closure path's typed observation
...
root can be relabeled as v30 evidence.
...
Schema v30 retains `execution.unsupported-event-source` as a twelfth mandatory
path and the host-owned machine-readable observation geometry introduced by
v6. Its internal `fn64.device-evidence.v18` channel retains schema v7's compact
...
v30 report, private admission, and paired verifier consume that stream as the
...
its host cannot mark them covered by declaration, and the schema-v30 report
...
identity. A closed v30 report is release evidence only when paired with that
...
independent of the schema-v30 wire shape and its geometry-bound encoding.
...
Schema v30 binds both paths through one typed descriptor. Reference capture can
...
but require schema-v30 regeneration and do not close the remaining
...
`x86_64-pc-windows-msvc`. Native Windows execution and positive Windows v30
...
edge, and captures all five schema-v30 channels plus the ABI-owned RSP/RDP
stream.
...
one exact schema-v30 `report_scenario`, private-input SHA-256, report SHA-256,
...
coarse platform coverage cannot manufacture that credit. A Windows v30 report
...
exactly ten schema-v30 reports, each paired with its terminal v3 journal and a
...
Schema v30 exposes normalized ROM identity, a host-supplied typed ROM class,
...
report both fail. The child's schema-v30 report template makes this authority-
...
exact v30 report scenario and semantic report SHA, exact ordered matrix run
...
Schema v30 still cannot expose blocker closure. Valid v30 evidence
...
recomputes every declaration SHA, reconstructs each retained v30 report and
...
All three scenarios and the joint matrix require schema-v30 regeneration. The
exact state is:
...
| OoT NTSC 1.0, Rust lane, reference LLE | Private host wiring, committed-VI capture, complete-RDRAM observation, an explicit source-hash-bound `BlockProgram` host-selection seam, an artifact/schema-bound whole-function entry stream, a create-new receipt/source-wire producer, runner-staged exact ROM/microcode-pair admission, and same-event kickoff check exist. The v30 report additionally owns memory/audio/trace bytes at the boundary, binds the unsupported-instrumentation denominator, and requires DeviceState v18 plus live timing v2. | Ten fresh schema-v22 processes completed and reverified on 2026-07-22 at cycle `722368695`; the retained private receipt binds their exact semantic report and ten run identities, but they are historical under v30. |
...
- Schema v30 aggregates the modeled device fabric, executor control,
