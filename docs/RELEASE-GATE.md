...
`fn64.release-gate.v25` schema, cycle, exact ordered channel set, byte lengths,
and channel hashes. Schema v25 emits each closure path's typed observation
...
For ROM input, v25 additionally binds the declared class, source z64/n64/v64
...
credit. On Windows, v25 also requires an exact native workstation identity:
...
rather than reinterpreted under the v25 device-authority wire.
The Memory
...
alone does not bind the scenario's private-input or environment (although v25's
...
schema. Moving from v24 to v25 intentionally changes both the artifact root
and `report_sha256`: v25 requires the expanded internal DeviceState v12 wire,
including DPC CLOCK, BUFBUSY, PIPEBUSY, and TMEM, so a retained v24 report
cannot be accepted under the new owner-state semantics. V24 added DeviceState
...
root can be relabeled as v25 evidence.
...
pre-v25 artifact root and contradictory closure states: unexercised means zero
...
Schema v25 retains `execution.unsupported-event-source` as a twelfth mandatory
path and the host-owned machine-readable observation geometry introduced by
v6. Its internal `fn64.device-evidence.v12` channel retains schema v7's compact guest register projection,
...
PIPEBUSY, and TMEM. Report construction freezes the same policy
...
future device result; their typed observations instead enter the canonical v25
...
boundary fail closed. The v25 RSP/RDP stream is likewise append-only release
...
v25 report, private admission, and paired verifier consume that stream as the
...
its host cannot mark them covered by declaration, and the schema-v25 report
...
identity. A closed v25 report is release evidence only when paired with that
...
independent of the schema-v25 wire shape and its geometry-bound encoding.
...
Schema v25 binds both paths through one typed descriptor. Reference capture can
...
Build evidence does not stand in for runtime microcode kickoff identity. Each v25
...
Retained v25 validation requires every task address to name a complete 64-byte
...
but require schema-v25 regeneration and do not close the remaining
...
`x86_64-pc-windows-msvc`. Native Windows execution and positive Windows v25
...
report emitted before the run. The checker compares it with every v25
...
edge, and captures all five schema-v25 channels plus the ABI-owned RSP/RDP
stream.
...
one exact schema-v25 `report_scenario`, private-input SHA-256, report SHA-256,
...
coarse platform coverage cannot manufacture that credit. A Windows v25 report
...
exactly ten schema-v25 reports, each paired with its terminal v3 journal and a
...
Schema v25 exposes normalized ROM identity, a host-supplied typed ROM class,
...
report both fail. The child's schema-v25 report template makes this authority-
...
exact v25 report scenario and semantic report SHA, exact ordered matrix run
...
Schema v25 still cannot expose blocker closure. Valid v25 evidence
...
recomputes every declaration SHA, reconstructs each retained v25 report and
...
platform-case authorities. V16 lacks v25's boundary-owned observations and
...
credit inside the previous canonical incomplete assessment. Schema v25 now
...
| --- | --- | --- |
| Synthetic fixtures and end-to-end runner | Five-channel fixed-cycle reports, v3 report/journal/run-event binding, real executor/device/RSP/RDP/VI/reference-render boundaries, derived matrix coverage, and the canonical incomplete assessment are available. | Ten retained public identified-native XBUS v22 reports historically supplied generic native/reference, no-cartridge-save, standard-controller, DRAM-DPC, and XBUS-DPC mechanism rows. They have no private-ROM authority or trusted private-series receipt, and require v25 regeneration; an incomplete assessment is not a complete retained matrix. |
| OoT NTSC 1.0, Rust lane, reference LLE | Private host wiring, committed-VI capture, complete-RDRAM observation, an explicit source-hash-bound `BlockProgram` host-selection seam, an artifact/schema-bound whole-function entry stream, a create-new receipt/source-wire producer, runner-staged exact ROM/microcode-pair admission, and same-event kickoff check exist. The v25 report additionally owns memory/audio/trace bytes at the boundary, binds the unsupported-instrumentation denominator, and requires DeviceState v12. | Ten fresh schema-v22 processes completed and reverified on 2026-07-22 at cycle `722368695`; the retained private receipt binds their exact semantic report and ten run identities, but they are historical under v25. |
...
series bind the same compiled unsupported-instrumentation denominator; no v25
...
- Schema v25 aggregates the modeled device fabric, executor control,
  ...
  v25 digests do not claim excluded
...
DeviceState v12 proves equality only for the modeled single-owner state. It
...
silicon bus/cache details. Those residuals remain explicit even when every v25
