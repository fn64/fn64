# fn64-cpu-runtime whole-ROM coverage baseline

This is a living, reproducible measurement of how much of a real N64 ROM the
all-Rust recompiler `fn64-cpu-runtime` recompiles today, and — more importantly —
...
```text
cargo build --release -p fn64-cpu-runtime --bin recompile_rom
...
- **Interpreter trap arms (done).** The interpreter
  (`crates/fn64-cpu-runtime/src/interp.rs`) now raises `break`/`syscall` and the
...
removed: once `fn64-cpu-runtime` became the in-tree recompiler, the adapter had no
