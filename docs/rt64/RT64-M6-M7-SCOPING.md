...
`docs/rt64/rt64-port-inventory.json` are literal-port cards, and neither milestone
...
`docs/rt64/rt64-port-authority.json`. No GPL runtime code was read or cited.
...
`docs/rt64/rt64-port-inventory.json` assigns 14 files (1,613 lines) to M6 and
...
lint-docs: 5 error(s)
  docs/rt64/RT64-RUNTIME-SHADER-CORPUS.md:25,27,28,29: asserts content hash … that no test checks
...
- **A real measurement wire schema, mechanically checked.**
  `docs/rt64/rt64-render-measurement-schema.json` + `tools/run_rt64_render_baseline.py`
...
- **No matched baseline, and the schema currently forbids one.**
  `docs/rt64/RT64-RENDER-MEASUREMENT.md:8-19` states report production is
...
- **M0.3 is RUNNING with reliability NOT MET.** From
  `docs/rt64/rt64-port-status.json`, ticket M0.3: `cargo nextest run -p fn64-abi
...
(`docs/rt64/RT64-RENDER-MEASUREMENT.md:53`) but no producer.
...
(`docs/rt64/RT64-RENDER-MEASUREMENT.md:28-31`; enforced at
...
- **A parity ladder that mirrors it and passes its checker.**
  `docs/rt64/rt64-port-parity.json`: 50 required rows — 24 `base_renderer` +
...
5. **The measurement schema mechanically forbids the report M6's G6 requires.**
   `docs/rt64/RT64-RENDER-MEASUREMENT.md:28-31` and
