...
(`docs/rt64/rt64-port-parity.json:71`) is `RT64_PASS` / `RUST_PENDING`, and its
...
(`docs/rt64/RT64-PORT-PARITY.md:100-103`). No other engine can produce it. A row
...
- Compute each digest yourself with `shasum -a 256` against the pinned checkout
  **and** cross-check it against `docs/rt64/rt64-port-inventory.json`'s
...
| Format | `rustfmt --edition 2021 <file>` — **per file only** | Never a crate root, never recursive: rustfmt follows `mod` children. A broad invocation on a dirty module root produced roughly 36,000 lines of noise in unrelated legacy modules (`docs/rt64/RT64-PORT-DASHBOARD.md:862`). |
...
`tools/rt64_port_inventory.py` rewrites `docs/rt64/rt64-port-inventory.json` and
`docs/rt64/RT64-PORT-INVENTORY.md` **wholesale from a tree snapshot**, so a per-card
...
- The regeneration recipe is in `docs/rt64/RT64-PORT-INVENTORY.md:10-11` and needs
