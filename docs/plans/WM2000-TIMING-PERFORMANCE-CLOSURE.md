...
| W02 | First-divergence comparator | W00 | **Direct-PIF candidate aligns the first three events; acknowledgement is the next divergence.** `gate_timing_diff` over the producer-neutral device-trace wire reports the first strict semantic divergence and refuses incompatible identity/schema/clock/scope, same-producer, empty, ambiguous-resolution, and truncated/aborted evidence. An explicit `0x80000400` public-debugger capture boundary aligns the two lanes before event emission. After correcting the SI status poll from reserved `0xA480001C` to public `SI_STATUS_REG` at `0xA4800018`, ten reference captures retained SI busy start at cycle zero, completion and MI-SI raise at cycle 4,616, and acknowledgement at cycle 5,296. A typed direct-PIF candidate running the generated `osInitialize` body now reproduces the first three events and cycles exactly, then fails to emit the reference SI acknowledgement. Trace interrupt dispatch/acknowledgement next and replace the external partial-adapter admission with an ABI-issued complete-replacement catalog before claiming production closure. The observed Mupen VI cadence conflicts with the public register formula and remains diagnostic, not timing-policy authority. |
