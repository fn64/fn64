...
| H06 | A startup pump performs pathological recompiled guest-resume volume and exhausts marginal audio runway after delivery activates. | **PI timing correction validated; resume-cost premise narrowed.** Programmed-domain/geometry timing reduced the roughly 69 ms/20,317-step spike to 26.433--30.396 ms across ten runs while preserving exact counts and presentation identity. Thread 6 still owns 20,894 resumes, including 10,447 instruction checkpoints and 10,347 blocking receives; timing distributed rather than removed that work. Profile/coalescing analysis now targets the linked whole-function checkpoint boundary. | Retain guest-authorized first-DMA delivery and exact device ordering. Coalesce only boundaries proven to expose no earlier due device event, runnable higher-priority thread, or guest-visible interrupt; preserve exact output/event identity and run the 20+ concurrency bar if ownership/interleaving changes. A latency buffer remains a separate typed fallback, never a guest clock source. |
