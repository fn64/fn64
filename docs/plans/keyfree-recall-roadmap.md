...
CORRECTION (same day): "no evidence" was measured with a 32-bit pointer
scan, which is blind to addresses assembled from two 16-bit immediates. Re-
measured for split `lui`+`addiu`/`ori` construction, **6 of the 66 ARE
constructed** in code (0x80028ba0 and 0x8002c290 twice each, plus
0x80000460, 0x8002dfa0, 0x8002ceb0, 0x8002db80). Those are live candidates
for dynamic dispatch and belong to the trace lane. A base+16-bit-offset
table test was also run and rejected as coincidence (12,265 "hits" for one
value across 32 MB of asset data); adjacent hi/lo halfword pairs: none.

For the remaining 60 no mechanism reaches them: no pointer anywhere in the
full 32 MB (not just the 4% that is code), no split construction, and no
compressed blob to hide one (WM2000 carries zero Yaz0/MIO0/Yay0 archives).
A dispatch table can only dispatch to an address it contains, so dynamic
calling cannot reach what nothing produces. So the sound key-free
ceiling on NWXE is 781-787/847 (92.2-92.9%), and M4's trace lane is worth exactly its
