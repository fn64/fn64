...
| `full-adapter-rom-coverage` | `base-rendering-accuracy` | A bounded native Metal VI gate observes exact nondefault post-VI pixels and proves fn64's seeded gamma-dither policy and coverage-gated horizontal divot are causal, repeatable, and restorable in pinned RT64. Dither-restoration and AA-selector output remain pixel-inert residuals; MSAA/downsample coverage semantics remain uncertified and bounded. Full-HLE/full-ROM fixed-cycle zero-unsupported coverage and physical-console VI parity are not established. |
