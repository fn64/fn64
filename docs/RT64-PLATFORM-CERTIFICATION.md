...
| `full-adapter-rom-coverage` | `base-rendering-accuracy` | A bounded native Metal VI gate observes exact nondefault post-VI pixels and names gamma-dither, divot, dither-restoration, and AA-selector output as pinned pixel-inert residuals. Full-HLE/full-ROM fixed-cycle zero-unsupported coverage and physical-console VI parity are not established. |
