...
|---|---|---|---|---:|---:|---|
| `macos-metal` | `platform-macos` (`open`) | `metal` | `metal-bgra8-unorm` | 13 pass / 0 blocked / 0 skipped | 7 open | Every required Metal case retains its applicable 10/20-run evidence, including HFR and Extended-GBI cooperation; user-controls/rebuild has a separate 20-clean-run race-fix result and enhancement/emulator controls retain bounded causal results on the same macOS 26.5 arm64 Apple-M5-Pro host. Both S2DEX controls additionally pass twenty fresh processes with complete load-program and exact pixel/RDRAM evidence. The changed native VI shader/cadence overlay passes inside the official watchdog-bounded backend-lifecycle runner in 20/20 fresh processes with the required macOS surface-teardown interval. Its twenty VI phases retain exact nondefault pixels, causal/restorable gamma and seeded gamma-dither, exact repeated-seed output across distinct presents, and different-seed divergence. Coverage-gated divot changes twelve exact componentwise-median pixels and restores exactly. DITHER_FILTER restoration changes eighteen exact full-coverage pixels, leaves twenty-four non-full controls and six flat full-coverage controls unchanged, preserves alpha, and restores exactly when disabled. Only AA-selector output remains a pixel-inert residual. The evidence remains bounded to the pinned-Metal nearest/native-scale/progressive synthetic RGBA16 fixture: managed-target RGB/dither history and coverage, linear/AAPS/enhancement-lattice, MSAA/downsample, other APIs, full-ROM, silicon, and analog behavior remain uncertified. All seven non-shrinking platform blockers remain open pending production full-ROM, physical VI evidence, and wider platform certification. |
...
| `full-adapter-rom-coverage` | `base-rendering-accuracy` | A bounded native Metal VI gate observes exact nondefault post-VI pixels and proves fn64's seeded gamma-dither policy, coverage-gated horizontal divot, and DITHER_FILTER RGBA16 restoration are causal and restorable in pinned RT64. Divot changes twelve exact componentwise-median pixels. Restoration changes eighteen exact full-coverage pixels while twenty-four non-full controls and six flat full-coverage controls remain unchanged and alpha is preserved. Only AA-selector output remains a pixel-inert residual. The fixture is nearest-filter, native-scale, progressive, synthetic RGBA16; managed-target RGB/dither history and coverage are not authoritative N64 state, and linear/AAPS/enhancement-lattice, MSAA/downsample, other-API, full-ROM, silicon, and analog behavior remain uncertified. Full-HLE fixed-cycle zero-unsupported coverage is not established. |
