...
| `full-adapter-rom-coverage` | `base-rendering-accuracy` | A bounded native Metal VI gate observes exact nondefault post-VI pixels and proves fn64's seeded gamma-dither policy is causal, repeatable, and restorable in pinned RT64. Divot, dither-restoration, and AA-selector output remain pixel-inert residuals. Full-HLE/full-ROM fixed-cycle zero-unsupported coverage and physical-console VI parity are not established. |
