...
//! `fn64-recomp-native` a *whole-program* recompiler and not a bag of
...
pub fn emit_lookup_dispatcher(symbols: &SymbolTable) -> String {
    ...
    out.push_str("        Err(_) => panic!(\"lookup: no native function or host shim at vram {vram:#010X}\"),\n");
    out.push_str("    }\n");
    out.push_str("}\n");
    out
}
...
pub fn emit_module(funcs: &[ModuleFunc], symbols: &SymbolTable) -> String {
    let mut out = String::new();
    out.push_str("// Generated by fn64-recomp-native (ELF/symbol front-end).\n");
