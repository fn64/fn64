...
//! a generated typed-Rust module out. Proves `NativeRecompiler` is a drop-in
...
use fn64_recomp_native::NativeRecompiler;
...
fn recompile_produces_typed_rust_module() {
    ...
    let dir = std::env::temp_dir().join(format!("fn64_native_{}", std::process::id()));
    ...
    let recomp = NativeRecompiler::new(AbiVersion::new(1, 0));
...
fn stubbed_function_is_skipped() {
    ...
    let dir = std::env::temp_dir().join(format!("fn64_native_stub_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let rom_path = dir.join("tiny.z64");
    std::fs::write(&rom_path, &rom).unwrap();
    ...
    let out = NativeRecompiler::default().recompile(&cfg).unwrap();
...
fn rsp_recompile_declines_loudly() {
    ...
    let err = NativeRecompiler::default().recompile_rsp(&cfg).unwrap_err();
