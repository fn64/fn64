...
pub use backend::{FullBackend, RawDpcBackend, RenderBackend, SettingsSink};
...
mod tests {
    ...
    impl RawDpcBackend for FakeBackend {}

    impl SettingsSink for FakeBackend {}
    ...
    fn is_dyn_safe_and_usable_through_a_trait_object() {
        let mut backend: Box<dyn FullBackend> = Box::new(fake(vec![UcodeId::F3dex2]));
