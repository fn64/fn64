...
Other-mode, remaining color registers, scissor, and sync ops can still be
acknowledged-and-skipped for this reference backend; texture/lighting and the
common combiner path above are implemented.
...
`G_SETTILE` and `G_SETTIMG`; the texture/combiner commands among these are now
handled as described in section 5, while remaining other-mode and sync ops are
still named skips.
