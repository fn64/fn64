...
/// Exact F3DEX2-compatible text images admitted for HLE. Admission applies
/// equally to the rspboot-populated task-entry image and to later self-load
/// targets: neither is inferred from the task header or a caller-selected
/// decode mode.
#[derive(Clone, Debug, Default)]
pub struct F3dex2UcodeCatalog {
    digests: HashSet<UcodeDigest>,
}

impl F3dex2UcodeCatalog {
    pub fn admit_sha256(&mut self, digest: [u8; 32]) {
        self.digests.insert(UcodeDigest::from_sha256(digest));
    }
    ...
    /// Return the exact identity of `text` only when the host explicitly
    /// admitted that image as F3DEX2-compatible.
    pub fn require_text(&self, text: &[u8]) -> Result<UcodeDigest, RenderError> {
        let digest = UcodeDigest::from_text(text);
    ...
    fn contains(&self, digest: UcodeDigest) -> bool {
...
/// the S/T coords + dsdx/dtdy. The ordered decoder consumes and preserves
/// both words; the reference executor implements non-flipped copy plus
/// one/two-cycle normal and flipped combiner/blender paths. Flipped copy
/// remains a named gap.
...
impl Texture {
    ...
    pub fn sample_filtered(&self, s: f32, t: f32, filter: TextureFilter) -> [u8; 4] {
        let shift = |coordinate: f32, encoded: u8| -> f32 {
            match encoded {
                0 => coordinate,
                1..=10 => coordinate / (1_u32 << encoded) as f32,
                11..=15 => coordinate * (1_u32 << (16 - encoded)) as f32,
                _ => unreachable!("G_SETTILE shift is a four-bit field"),
            }
        };
        let address = |coordinate: i64, dim: u32, clamp: bool, mirror: bool, mask: u8| -> u32 {
            if dim == 0 {
                return 0;
            }

            // Mask zero forces clamping. With a nonzero mask, the explicit
            // clamp bit clamps the incoming coordinate before mirror/mask;
            // this ordering reproduces the manual's SH=11/mask=2 example,
            // where every input above 11 resolves to mirrored texel 3.
            let coordinate = if mask == 0 || clamp {
                coordinate.clamp(0, dim as i64 - 1)
            } else {
                coordinate
            };
            if mask == 0 {
            ...
            let coordinate = if mirror && coordinate & (1_i64 << mask) != 0 {
                (!coordinate) & low_mask
            } else {
                coordinate & low_mask
            };
            assert!(
                self.tmem.is_some() || coordinate < i64::from(dim),
                "G_SETTILE mask {mask} addresses texel {coordinate} outside decoded {dim}-texel fixture"
            );
            coordinate as u32
        };
        let texel = |x: i64, y: i64| -> [u8; 4] {
            let x = address(x, self.width, self.clamp_s, self.mirror_s, self.mask_s);
            let y = address(y, self.height, self.clamp_t, self.mirror_t, self.mask_t);
        ...
        let s = shift(s, self.shift_s) - self.origin_s;
        let t = shift(t, self.shift_t) - self.origin_t;
        ...
        std::array::from_fn(|channel| {
            let [c00, c10, c01, c11] = samples.map(|sample| sample[channel] as f32);
            let value = if sf + tf <= 1.0 {
                c00 + sf * (c10 - c00) + tf * (c01 - c00)
            } else {
                c11 + (1.0 - sf) * (c01 - c11) + (1.0 - tf) * (c10 - c11)
            };
...
/// One fill-cycle rectangle with all state required at its command position.
#[derive(Copy, Clone, Debug, PartialEq)]
        ...
        match offset {
            G_MWO_CLIP_RNX | G_MWO_CLIP_RNY => {
                let ratio = u8::try_from(low).unwrap_or_else(|_| {
                    panic!("G_MOVEWORD G_MW_CLIP negative-side ratio {low:#06x} is not FRUSTRATIO_1..6")
                });
                assert!(
                    (1..=6).contains(&ratio),
...
fn reset_rsp_state_from_ucode_load(state: &mut DecodeState) {
    state.vtx_cache = [Vertex::default(); 32];
    state.mvp = None;
...
impl Tmem {
    ...
    fn read_tlut(&self, index: usize, mode: u8) -> [u8; 4] {
        ...
        match mode {
            ...
            _ => panic!("CI texture sampled with texture-LUT mode {mode}, expected RGBA16 or IA16"),
        }
    }
}
...
impl TmemTexture {
    fn sample(&self, x: usize, y: usize) -> [u8; 4] {
        ...
        // EN_TLUT is a pipeline mode, not a tile-format property: when the
        // other-mode TLUT enable is set, EVERY 4-bit texel becomes a
        // palette-relative index and every 8-bit texel a direct index into
        // high-TMEM, regardless of the tile's declared format (angrylion
...
fn decode_display_list_f3dex2_state(
    ...
    decode_stream(&mut scratch, dl_addr, &mut state, None, None);
    #[cfg(not(test))]
    projdump::summary();
    Ok(state)
}
...
fn execute_display_list_f3dex2_state_with_catalog(
    ...
    execute_display_list_f3dex2_state_with_catalog_and_rdp(
        rdram, rsp_memory, dl_addr, catalog, None,
    )
}
...
fn execute_display_list_f3dex2_state_with_catalog_and_rdp(
    ...
    #[cfg(not(test))]
    projdump::reset_frame();
    decode_stream(rdram, dl_addr, &mut state, Some(rsp_memory), catalog);
    #[cfg(not(test))]
    projdump::summary();
...
pub fn decode_raw_rdp_ops(rdram: &[u8], start: u32) -> Result<Vec<RenderOp>, RenderError> {
    let mut state = fresh_decode_state();
    let mut scratch = rdram.to_vec();
    decode_stream_impl(&mut scratch, start, &mut state, true, None, None);
    Ok(state.ops)
}
...
pub fn validate_raw_rdp_command_range(
    ...
    let mut pc = start;
    while pc < end {
        let opcode = (read_u32(rdram, pc as usize) >> 24) as u8;
...
fn decode_stream(
    ...
    decode_stream_impl(rdram, dl_addr, state, false, rsp_memory, ucode_catalog);
}
...
fn decode_stream_impl(
    ...
    loop {
        let command_end = pc.checked_add(8).unwrap_or_else(|| {
            panic!("F3DEX2 display-list PC {pc:#010x} overflows the host address space")
        });
        assert!(
            command_end <= rdram.len(),
            "F3DEX2 display list is truncated at RDRAM {pc:#010x}: need 8 command bytes, rdram_bytes={}",
            rdram.len()
        );
        state.cmds_decoded += 1;
        assert!(
            state.cmds_decoded <= MAX_DL_COMMANDS,
            "F3DEX2 display list exceeded the {MAX_DL_COMMANDS}-command budget at RDRAM {pc:#010x}; cyclic or corrupt command graph"
        );
        ...
        match opcode {
            ...
            G_CULLDL => {
                ...
                assert!(
                    start < end && end < state.vtx_cache.len(),
                    "G_CULLDL inclusive cache range {start}..={end} must satisfy 0 <= start < end <= 31"
                );
            ...
            G_BRANCH_Z => {
                ...
                let vertex = state.vtx_cache.get(vertex_slot).unwrap_or_else(|| {
                    panic!("G_BRANCH_Z cache slot {vertex_slot} is outside F3DEX2 slots 0..=31")
                });
            ...
            G_VTX => {
                ...
                assert!(
                    (1..=state.vtx_cache.len()).contains(&n),
                    "G_VTX count {n} must be in 1..={} at RDRAM {:#010x}",
                    state.vtx_cache.len(),
                    pc - 8
                );
                assert!(
                    end <= state.vtx_cache.len() && end >= n,
            ...
            G_MODIFYVTX => {
                ...
                let vertex = state.vtx_cache.get_mut(slot).unwrap_or_else(|| {
                    panic!("G_MODIFYVTX cache slot {slot} is outside F3DEX2 slots 0..=31")
                });
                match where_field {
                    G_MWO_POINT_RGBA => {
            ...
            G_DMA_IO => {
                let rsp_memory = rsp_memory.as_deref_mut().unwrap_or_else(|| {
                    panic!(
                        "G_DMA_IO requires execute_display_list_f3dex2_ops with live RSP memory"
                    )
                });
            ...
            G_LOAD_UCODE => {
                ...
                if ucode_catalog.is_some_and(|catalog| !catalog.contains(digest)) {
            ...
            G_MOVEWORD => {
                ...
                if index == G_MW_SEGMENT {
                    ...
                    assert_eq!(
                        offset, 0,
                        "G_MOVEWORD G_MW_NUMLIGHT offset must be zero"
                    );
                    assert!(
            ...
            G_TEXRECT | G_TEXRECTFLIP => {
                // Public gSPTextureRectangle is a 16-byte command: the first
                // Gfx holds screen bounds/tile and the second holds signed
                // S10.5 texture origins plus signed S5.10 gradients.
                let second_word_end = pc.checked_add(8).unwrap_or_else(|| {
                    panic!(
                        "{} second-word PC {pc:#010x} overflows the host address space",
                        opcode_name(opcode)
                    )
                });
                assert!(
                    second_word_end <= rdram.len(),
                    "{} is truncated at RDRAM {pc:#010x}: need 8 second-word bytes, rdram_bytes={}",
                    opcode_name(opcode),
                    rdram.len()
                );
                let coords = read_u32(rdram, pc);
                let gradients = read_u32(rdram, pc + 4);
                pc += 8;
            ...
            G_SPECIAL_1 | G_SPECIAL_2 | G_SPECIAL_3 => panic!(
                "reserved F3DEX2 command {} at RDRAM {:#010x}: w0={w0:#010x} w1={w1:#010x}",
                opcode_name(opcode),
                pc - 8
            ),
            G_ENDDL => break,
            _ => panic!(
                "unsupported F3DEX2 command {} ({opcode:#04x}) at RDRAM {:#010x}: w0={w0:#010x} w1={w1:#010x}",
                opcode_name(opcode),
                pc - 8
            ),
        }
...
fn texture_for_tile(
    ...
    let (uls, ult, lrs, lrt) = (tile.uls / 4, tile.ult / 4, tile.lrs / 4, tile.lrt / 4);
    if lrs < uls || lrt < ult {
        return None;
    }
    let width = u32::from(lrs - uls + 1);
    let height = u32::from(lrt - ult + 1);
...
fn resolve_tri(
    ...
    assert!(
        idx.iter().all(|&i| (i as usize) < vtx_cache.len()),
...
fn clip_line_to_homogeneous_volume(
    ...
    let (Some(mut start_clip), Some(mut end_clip)) = (start.clip_position, end.clip_position)
        ...
        assert!(
            !behind_near_plane(&start) && !behind_near_plane(&end),
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "unsupported F3DEX2 command G_<unrecognized> (0x09)")]
    ...
    fn fog_geometry_mode_replaces_vertex_alpha_from_projected_depth() {
        ...
        load_vertices(&rdram, &mut state, base as u32, 3, 0);
    ...
    fn texture_generation_replaces_explicit_vertex_coordinates() {
        ...
        load_vertices(&rdram, &mut state, 0, 1, 0);
