...
use fn64_cpu_runtime_codegen::RsRecompiler;
...
fn recompile_produces_typed_rust_module() {
    ...
    let dir = std::env::temp_dir().join(format!("fn64_cpu_runtime_{}", std::process::id()));
...
fn stubbed_function_is_skipped() {
    ...
    let dir = std::env::temp_dir().join(format!("fn64_cpu_runtime_stub_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let rom_path = dir.join("tiny.z64");
    std::fs::write(&rom_path, &rom).unwrap();
    ...
    let _ = std::fs::remove_dir_all(
        std::env::temp_dir().join(format!("fn64_cpu_runtime_stub_{}", std::process::id())),
    );
}
