...
// is the emitter output's recompiled path except that the host-first wrapper and
...
fn emitter_output_matches_mixed_golden() {
    ...
    assert!(
        out.contains("call_host_or_recompiled(0x8006385C, Math_StepToF, ctx, mem);"),
...
fn mixed_isa_execution_matches_oracle() {
    ...
    for &actor_f in &floats {
        for &hword in &hwords {
            let mut buf = vec![0u8; fn64_cpu_runtime::RDRAM_LEN];
