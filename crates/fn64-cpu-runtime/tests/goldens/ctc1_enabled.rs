...
pub fn ctc1_enabled_recomp(ctx: &mut RecompContext, mem: &mut Rdram) {
    fn64_cpu_runtime::notify_function_entry(fn64_cpu_runtime::TranslatedFunctionIdentity::new(0x80110000, "ctc1_enabled_recomp"));
    let mut pc: u32 = 0x80110000;
    'run: loop { match pc {
        0x80110000 => {
            // 0x80110000: Ctc1 { rt: 2, fs: 31 }
            ctx.write_fcr(31, ctx.r_u32(2));
            if ctx.fcsr_exception_pending() { fn64_cpu_runtime::trap_unsupported("enabled FCSR cause written by CTC1 in whole-function lane"); }
            // 0x80110004: Jr { rs: 31 }
