...
pub fn synth_recomp(ctx: &mut RecompContext, mem: &mut Rdram) {
    fn64_cpu_runtime::notify_function_entry(fn64_cpu_runtime::TranslatedFunctionIdentity::new(0x80100000, "synth_recomp"));
    ...
    'run: loop { match pc {
        0x80100000 => {
            ...
            if ctx.fpu_mul_s(8, 4, 6) { fn64_cpu_runtime::trap_unsupported("enabled COP1 exception"); }
            // 0x80100010: AddS { fd: 0, fs: 8, ft: 4 }
            if ctx.fpu_add_s(0, 8, 4) { fn64_cpu_runtime::trap_unsupported("enabled COP1 exception"); }
