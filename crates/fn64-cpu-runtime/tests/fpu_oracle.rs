//! COP1 / FPU oracle-validation + decoder tests for `fn64-cpu-runtime`.
//!
...
fn compile_and_run_whole_function(emitted: &str, main_body: &str) -> String {
    let source = format!(
        r#"use fn64_cpu_runtime::{{Rdram, RecompContext}};
        ...
        match pc {
            0x80100000 => {
                ...
                if ctx.fpu_mul_s(8, 4, 6) {
                    fn64_cpu_runtime::trap_unsupported("enabled COP1 exception");
                }
...
fn floating_point_exception_vectors_to_exccode_15() {
    use fn64_cpu_runtime::{BankId, CpuException, CpuFault, CpuFaultKind, ExecutionKey, GuestPc};
...
fn conditional_moves_decode() {
    use fn64_cpu_runtime::Instruction::*;
