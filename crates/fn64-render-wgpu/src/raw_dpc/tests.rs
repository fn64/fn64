...
fn packet_with_tmem_sources_in_layout(
    ...
    accesses.extend(
        source_ranges
            .iter()
            .enumerate()
            .map(|(index, &(start, end))| tmem_source_access(layout, index as u32 + 1, start, end)),
    );
        ...
        destination_ranges
            ...
            .map(|(index, &(start, end))| {
                ResourceAccess::try_new(
                    ...
                    ResourceRegion::Tmem(fn64_render_ir::TmemRange::try_new(start, end).unwrap()),
                )
                .unwrap()
            }),
    );
    ...
    for range in projected {
        if let Some(last) = canonical.last_mut() {
            if range.start() <= last.end() {
                *last =
                    fn64_render_ir::TmemRange::try_new(last.start(), last.end().max(range.end()))
                        .unwrap();
                continue;
            }
        }
...
fn the_three_wm2000_blocking_ids_decode_as_state_free_noops() {
    for prefix in [0x00u8, 0x40, 0x80, 0xc0] {
        for id in [SYNC_PIPE, SYNC_TILE, STREAM_TERMINATOR] {
            ...
            let submitted = submit(packet(7, words, &[]));
            let decoded = decode_raw_dpc(submitted, &RdpState::default()).unwrap_or_else(|error| {
...
fn tmem_state_commands_decode_every_public_field_width_for_all_prefixes() {
    for prefix in [0x00, 0x40, 0x80, 0xc0] {
        let words = vec![
            ...
            7 << 24 | 0x0f << 20 | 3 << 18 | 0x0a << 14 | 0x0b << 10 | 1 << 8 | 0x0c << 4 | 0x0d,
...
fn tmem_source_plans_reject_cross_decode_range_and_layout_aliases() {
    ...
    let RawDpcCommandKind::LoadBlock(different_range_load) = different_range.commands()[3].kind()
    else {
        panic!("expected LoadBlock");
    };
    assert_ne!(
    ...
    let RawDpcCommandKind::LoadBlock(different_layout_load) = different_layout.commands()[3].kind()
    else {
        panic!("expected LoadBlock");
    };
    assert_ne!(
...
fn set_scissor_words(prefix: u8, mode: u32, ulx: u32, uly: u32, lrx: u32, lry: u32) -> Vec<u32> {
    vec![
