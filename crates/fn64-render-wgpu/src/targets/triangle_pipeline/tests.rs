...
fn p2_schedule(payload_bytes: usize) -> ComputeP2PreparedSchedule {
    ComputeP2PreparedSchedule {
        status_bytes: 0,
        word_count: 0,
        workgroups: 0,
        checkpoint_output_count: 0,
        checkpoint_patches: Vec::new(),
        work_items: vec![0; payload_bytes],
        work_triangle_indices: Vec::new(),
        work_item_words: Vec::new(),
    }
}

fn p2_schedule_key() -> ComputeP2PlanKey {
    ComputeP2PlanKey {
        ...
        checkpoint_dispatch_limits: vec![1, 2],
    }
}

#[test]
fn p2_schedule_cache_full_equality_classifies_every_structural_miss_field() {
    let base = p2_schedule_key();
    let mut cache = ComputeP2PlanCache::default();
    assert!(cache.lookup(&base).is_none());
    cache.insert(base.clone(), p2_schedule(0)).unwrap();
    assert!(cache.lookup(&base).is_some());

    let mut mutations = Vec::new();
    for mutate in [
        |key: &mut ComputeP2PlanKey| key.extent.width += 1,
        |key: &mut ComputeP2PlanKey| key.extent.height += 1,
        |key: &mut ComputeP2PlanKey| key.dispatch_start += 1,
        |key: &mut ComputeP2PlanKey| {
            key.dispatches.pop();
        },
        |key: &mut ComputeP2PlanKey| key.dispatches.swap(0, 1),
        |key: &mut ComputeP2PlanKey| key.dispatches[0].first_row += 1,
        |key: &mut ComputeP2PlanKey| key.dispatches[0].row_count += 1,
        |key: &mut ComputeP2PlanKey| key.dispatches[0].first_column += 1,
        |key: &mut ComputeP2PlanKey| key.dispatches[0].column_count += 1,
        |key: &mut ComputeP2PlanKey| key.dispatches[0].triangle_count += 1,
        |key: &mut ComputeP2PlanKey| {
            key.checkpoint_dispatch_limits.pop();
        },
        |key: &mut ComputeP2PlanKey| key.checkpoint_dispatch_limits[0] += 1,
        |key: &mut ComputeP2PlanKey| key.checkpoint_dispatch_limits.reverse(),
    ] {
    ...
    for mutation in mutations {
        assert!(cache.lookup(&mutation).is_none());
        assert!(cache.lookup(&base).is_some());
    }

    assert_eq!(
        cache.counters,
        ComputeP2PlanCacheCounters {
            attempts: 28,
            hits: 14,
            cold_misses: 1,
            extent_misses: 2,
            partition_misses: 1,
            dispatch_range_misses: 6,
            triangle_count_misses: 1,
            checkpoint_misses: 3,
            ..ComputeP2PlanCacheCounters::default()
        }
    );
}

#[test]
fn p2_schedule_cache_is_byte_bounded_and_preserves_the_recent_hit() {
    let mut first = p2_schedule_key();
    first.extent.width = 8;
    let mut second = first.clone();
    second.extent.width = 9;
    let mut third = first.clone();
    third.extent.width = 10;
    let entry_bytes = first.retained_bytes() + p2_schedule(64).retained_bytes();
    let mut cache = ComputeP2PlanCache::with_max_retained_bytes(entry_bytes * 2);
    cache.insert(first.clone(), p2_schedule(64)).unwrap();
    cache.insert(second.clone(), p2_schedule(64)).unwrap();
    assert!(cache.lookup(&first).is_some());
    cache.insert(third, p2_schedule(64)).unwrap();

    assert!(
        cache.lookup(&first).is_some(),
        "the recent hit must survive"
    );
    assert!(cache.lookup(&second).is_none(), "the LRU entry must evict");
    assert_eq!(cache.counters.evictions, 1);
    cache.insert(second.clone(), p2_schedule(64)).unwrap();
    assert!(
        cache.lookup(&second).is_some(),
        "an evicted key must rebuild"
    );
...
#[test]
fn p2_schedule_builder_matches_aligned_and_odd_checkpoint_goldens() {
    let projection = TmemGpuProjection {
        bytes: [0; fn64_render_ir::TMEM_BYTES as usize],
        validity_words: [0; crate::TMEM_VALIDITY_WORDS],
    };
    ...
    let raw =
        crate::RawTriangle::decode(crate::wire_words::RAW_TRIANGLE_BASE_EDGE, &triangle_bytes)
            .unwrap();
    let triangles = [
        ComputeCoverageTriangle::from_raw(raw).with_program(ComputeRasterShaderProgram::Fog),
        ComputeCoverageTriangle::from_raw(raw).with_program(ComputeRasterShaderProgram::Fog),
    ];
    let aligned_dispatch = ComputeHotColorDispatch {
    ...
    let mut scratch = ComputeHotColorPlanScratch::default();
    let aligned = build_compute_hot_color_chain_schedule(
        TriangleTargetExtent {
            width: 4,
            height: 2,
        },
        4,
        0,
        &[(&aligned_dispatch, 0, 1)],
        &[1],
        &mut scratch,
    )
    .unwrap();
    assert_eq!(aligned.word_count, 4);
    assert_eq!(aligned.status_bytes, 32);
    assert_eq!(aligned.workgroups, 1);
    assert_eq!(aligned.checkpoint_output_count, 4);
    assert_eq!(
        aligned.checkpoint_patches,
        [vec![(0, 0), (1, 1), (2, 2), (3, 3)]]
    );
    assert_eq!(aligned.work_item_words, [0, 1, 2, 3]);
    assert_eq!(
        aligned
    ...
    let odd = build_compute_hot_color_chain_schedule(
        TriangleTargetExtent {
            width: 3,
            height: 2,
        },
        3,
        0,
        &[(&odd_dispatches[0], 0, 1), (&odd_dispatches[1], 1, 1)],
        &[1, 2],
        &mut scratch,
    )
    .unwrap();
    assert_eq!(
        odd.checkpoint_patches,
        [vec![(0, 0), (1, 1)], vec![(1, 2), (2, 3)]]
    );
    assert_eq!(odd.work_item_words, [0, 1, 2]);
    assert_eq!(
        odd.work_items
            .chunks_exact(4)
            .map(|bytes| u32::from_le_bytes(bytes.try_into().unwrap()))
