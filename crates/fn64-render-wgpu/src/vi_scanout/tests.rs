...
/// RGBA16 bit 0 is stored coverage bit 2, not primitive alpha. The generic
/// target oracle has no render-mode or geometric sample-mask input, so it
/// explicitly supplies full coverage. Two pixels straddling alpha 0x7f/0x80
/// must therefore pack the same low bit and VI must reconstruct both as full
/// coverage (Programming Manual §§15.5.3, 15.5.6, 15.7).
#[test]
fn this_lanes_rgba16_low_bit_is_a_coverage_msb_not_alpha() {
    ...
    .unwrap();
    let registry = crate::targets::ColorTargetRegistry::try_new(layout, 1).unwrap();
    let candidate = registry.begin_candidate(key).unwrap();
    let pixels = [
        crate::targets::Rgba8::new(0, 0, 0, 0x7f),
        crate::targets::Rgba8::new(0, 0, 0, 0x80),
    ];
    let packed = crate::targets::pack_device_pixels(&candidate, &pixels).unwrap();
    assert_eq!(packed.device_bytes(), [0x00, 0x01, 0x00, 0x01]);
    ...
    let mut rdram = fresh_rdram();
    write_rgba16(&mut rdram, 0, u16::from_be_bytes([0x00, 0x01]));
    write_rgba16(&mut rdram, 2, u16::from_be_bytes([0x00, 0x01]));
    let memory = fn64_runtime::PhysicalRdramRead::from_storage(&rdram);
    assert_eq!(geometry.coverage(&memory, 0, 0), 8);
    assert_eq!(geometry.coverage(&memory, 1, 0), 8);
}
