...
fn install_sram_handle(rdram: &mut [u8], offset: u32, timing: fn64_runtime::PiDomainTiming) -> u64 {
    let handle_vram = 0x8000_0000 | offset;
    unsafe {
        write_epi_handle(
            rdram.as_mut_ptr(),
...
fn interactive_rsp_rdp_retention_is_constant_space_and_counted() {
    ...
    load_rom(vec![1]);
    crate::set_rsp_rdp_observation_retention(crate::RspRdpObservationRetention::CompleteEvidence);
    assert!(crate::copy_rsp_rdp_observations().is_empty());
}
...
#[test]
fn raw_pif_terminate_boot_retains_exact_occurrence_until_cpu_acceptance() {
    load_rom(vec![0; 0x100]);
    with_host(|host| {
        host.device_fabric
    ...
    assert!(write_raw_mmio_word(0xFFFF_FFFF_BFC0_07FC, 0x0000_0008));
    ...
    assert_eq!(
        ...
        Some(1 << 12),
        "enabling MI-SI must not bypass architectural CPU acceptance"
    );
    assert_eq!(
        peripherals_evidence_snapshot()
            .pending_host_interrupt_routes
...
thread_local! {
    static LEGACY_C_DIRECT_PIF_MARKER: std::cell::Cell<u32> = const { std::cell::Cell::new(0) };
}

#[cfg(not(feature = "recomp-rs"))]
unsafe extern "C" fn legacy_c_direct_pif_interrupt_entry(rdram: *mut u8, ctx: *mut RecompContext) {
    let ctx = unsafe { &mut *ctx };
    ctx.r4 = u64::from(1 | (1 << 10) | (fn64_runtime::InterruptSource::Si.bit() << 16));
    unsafe { crate::osSetIntMask_recomp(rdram, ctx) };
    LEGACY_C_DIRECT_PIF_MARKER.with(|marker| marker.set(1));

    crate::fn64_c_mmio_write_w(0xFFFF_FFFF_BFC0_07FC, 0x0000_0008);
    LEGACY_C_DIRECT_PIF_MARKER.with(|marker| marker.set(2));

    let _ = crate::fn64_c_mmio_read_w(0xFFFF_FFFF_A430_0008);
    LEGACY_C_DIRECT_PIF_MARKER.with(|marker| marker.set(3));
}

#[test]
#[cfg(not(feature = "recomp-rs"))]
fn legacy_c_lane_samples_direct_pif_before_guest_continuation() {
    ...
    LEGACY_C_DIRECT_PIF_MARKER.with(|marker| marker.set(0));
    let mut rdram = vec![0u8; 0x1000];
    unsafe {
        crate::boot_thread0(
            rdram.as_mut_ptr(),
            rdram.len(),
            legacy_c_direct_pif_interrupt_entry,
            79,
            10,
        )
    };
    ...
    assert_eq!(
        LEGACY_C_DIRECT_PIF_MARKER.with(std::cell::Cell::get),
        1,
        "the charged write checkpoint must precede the PIF command"
    );
    ...
    assert_eq!(
        LEGACY_C_DIRECT_PIF_MARKER.with(std::cell::Cell::get),
        2,
        "the charged read checkpoint must park after the PIF command"
    );
    assert_eq!(
        peripherals_evidence_snapshot()
            .pending_host_interrupt_routes
    ...
    assert_ne!(
        device_snapshot().mi_pending & fn64_runtime::InterruptSource::Si.bit(),
        0
    );

    let accepted_at = sim_time();
    assert!(crate::run_one_step());
    assert_eq!(
        LEGACY_C_DIRECT_PIF_MARKER.with(std::cell::Cell::get),
        2,
        "guest C continued after accepting HostKernel work"
    );
    assert_eq!(
        with_executor(|executor| executor.host_kernel_work_deadline()),
        Some(fn64_runtime::EmulatedInstant::new(accepted_at + 536))
    );

    assert!(crate::run_one_step());
    assert_eq!(sim_time(), accepted_at + 536);
    assert_eq!(
        LEGACY_C_DIRECT_PIF_MARKER.with(std::cell::Cell::get),
        3,
        "guest C did not continue after exact acknowledgement"
    );
    assert_eq!(
        device_snapshot().mi_pending & fn64_runtime::InterruptSource::Si.bit(),
        0
    );
    assert!(peripherals_evidence_snapshot()
        .pending_host_interrupt_routes
        .is_empty());
    assert!(with_executor(|executor| executor
        .host_kernel_work_deadline()
        .is_none()));
}

#[test]
#[cfg(not(feature = "recomp-rs"))]
fn first_typed_resume_cannot_sample_parked_legacy_c_status() {
    with_executor(|executor| *executor = fn64_runtime::Executor::new());
    load_rom(vec![0; 0x100]);
    with_host(|host| {
        host.device_fabric
    ...
    assert!(crate::run_one_step());
    assert!(crate::run_one_step());
    assert_eq!(LEGACY_C_DIRECT_PIF_MARKER.with(std::cell::Cell::get), 2);
    assert!(host_interrupt_occurrence_ready().is_some());

    let typed_marker = std::rc::Rc::new(std::cell::Cell::new(0));
    let body_marker = std::rc::Rc::clone(&typed_marker);
    let rdram_addr = rdram.as_mut_ptr() as usize;
    with_executor(|executor| {
        executor.create_thread_with_saved_rcp_interrupt_mask(
            80,
            20,
            fn64_runtime::SavedRcpInterruptMask::from_bits(0).unwrap(),
            move |yielder, _| {
                crate::with_active_yielder(80, rdram_addr as *mut u8, yielder, || {
                    crate::sample_legacy_c_host_interrupt_boundary();
                    body_marker.set(1);
                });
            },
        );
        executor.start_thread(80);
    });
    ...
    assert_eq!(
        typed_marker.get(),
        1,
        "typed B sampled parked legacy A's enabled Status on its first resume"
    );
    assert!(with_executor(|executor| executor
        .host_kernel_work_deadline()
        .is_none()));
    assert_eq!(
        peripherals_evidence_snapshot()
            ...
            .len(),
        1,
        "typed B accepted legacy A's retained occurrence despite a closed saved gate"
    );
    assert!(crate::run_one_step());
    assert_eq!(
        LEGACY_C_DIRECT_PIF_MARKER.with(std::cell::Cell::get),
        2,
        "legacy A continued before its accepted HostKernel work committed"
    );
    assert!(crate::run_one_step());
    assert_eq!(LEGACY_C_DIRECT_PIF_MARKER.with(std::cell::Cell::get), 3);
    with_executor(|executor| *executor = fn64_runtime::Executor::new());
}

#[test]
fn generated_c_mi_mask_command_survives_thread_switch_round_trip() {
    with_executor(|executor| *executor = fn64_runtime::Executor::new());
    load_rom(vec![0; 0x100]);
    let mut rdram = vec![0u8; 0x1000];
    let rdram_addr = rdram.as_mut_ptr() as usize;
    let source_bit = fn64_runtime::InterruptSource::Sp.bit();
    let a_marker = std::rc::Rc::new(std::cell::Cell::new(0));
    let a_body_marker = std::rc::Rc::clone(&a_marker);
    with_executor(|executor| {
        executor.create_thread_with_saved_rcp_interrupt_mask(
            81,
            10,
            fn64_runtime::SavedRcpInterruptMask::from_bits(0).unwrap(),
            move |yielder, _| {
                crate::with_active_yielder(81, rdram_addr as *mut u8, yielder, || {
                    crate::fn64_c_mmio_write_w(0xFFFF_FFFF_A430_000C, 1 << 1);
                    a_body_marker.set(1);
                    assert_eq!(
                        yielder.suspend(fn64_runtime::Yield::PauseSelf),
                        fn64_runtime::Resume::Continue
                    );
                    assert_eq!(
                        u32::from(crate::live_saved_rcp_interrupt_mask().bits()),
                        source_bit,
                        "A's raw MI command was replaced by its stale saved mask"
                    );
                    a_body_marker.set(2);
                });
            },
        );
        executor.start_thread(81);
    });

    assert!(crate::run_one_step());
    assert_eq!(a_marker.get(), 0, "raw MMIO charge did not checkpoint");
    assert!(crate::run_one_step());
    assert_eq!(a_marker.get(), 1);
    assert_eq!(
        u32::from(crate::live_saved_rcp_interrupt_mask().bits()),
        source_bit
    );

    let b_marker = std::rc::Rc::new(std::cell::Cell::new(0));
    let b_body_marker = std::rc::Rc::clone(&b_marker);
    with_executor(|executor| {
        executor.create_thread_with_saved_rcp_interrupt_mask(
            82,
            20,
            fn64_runtime::SavedRcpInterruptMask::from_bits(0).unwrap(),
            move |yielder, _| {
                crate::with_active_yielder(82, rdram_addr as *mut u8, yielder, || {
                    assert_eq!(crate::live_saved_rcp_interrupt_mask().bits(), 0);
                    b_body_marker.set(1);
                });
            },
        );
        executor.start_thread(82);
    });

    assert!(crate::run_one_step());
    assert_eq!(b_marker.get(), 1);
    assert!(crate::run_one_step());
    assert_eq!(a_marker.get(), 2);
    with_executor(|executor| *executor = fn64_runtime::Executor::new());
}

#[test]
fn outer_scheduler_commits_exact_direct_pif_work_before_guest_resume() {
    with_executor(|executor| *executor = fn64_runtime::Executor::new());
    load_rom(vec![0; 0x100]);
    with_host(|host| {
        host.device_fabric
    ...
    advance_virtual_time(sim_time() + 1);
    set_mi_interrupt_mask(fn64_runtime::InterruptSource::Si.bit());
    let occurrence = host_interrupt_occurrence_ready().expect("SI occurrence must be ready");
    let accepted_at = sim_time();
    let marker = std::rc::Rc::new(std::cell::Cell::new(0));
    let body_marker = std::rc::Rc::clone(&marker);
    with_executor(|executor| {
        executor.create_thread_with_saved_rcp_interrupt_mask(
            77,
            10,
            fn64_runtime::SavedRcpInterruptMask::from_bits(
                fn64_runtime::InterruptSource::Si.bit() as u8
            )
            .unwrap(),
            move |yielder, _| {
                body_marker.set(1);
                assert_eq!(
                    yielder.suspend(fn64_runtime::Yield::HostInterruptAccepted {
                        occurrence,
        ...
        executor.start_thread(77);
    });

    assert!(crate::run_one_step());
    assert_eq!(marker.get(), 1, "accepted work must park the guest");
    assert_eq!(
        with_executor(|executor| executor.host_kernel_work_deadline()),
        Some(fn64_runtime::EmulatedInstant::new(accepted_at + 536))
    );
    ...
    assert!(crate::run_one_step());
    assert_eq!(sim_time(), accepted_at + 536);
    assert_eq!(marker.get(), 2, "guest resumed before exact work commit");
    assert!(with_executor(|executor| executor
        .host_kernel_work_deadline()
        .is_none()));
    assert!(peripherals_evidence_snapshot()
...
#[test]
fn rom_reload_retires_active_direct_pif_work_with_its_route() {
    with_executor(|executor| *executor = fn64_runtime::Executor::new());
    load_rom(vec![0; 0x100]);
    with_host(|host| {
        host.device_fabric
    ...
    with_executor(|executor| {
        executor.create_thread_with_saved_rcp_interrupt_mask(
            ...
            .unwrap(),
            move |yielder, _| {
                let _ = yielder.suspend(fn64_runtime::Yield::HostInterruptAccepted {
                    occurrence,
                    profile: fn64_runtime::HostKernelAdapterProfile::N64RecompLibultraV1,
                    service_class: fn64_runtime::HostKernelServiceClass::DirectPifSi,
                });
