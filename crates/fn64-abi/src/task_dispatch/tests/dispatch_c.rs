...
fn rspboot_waits_only_for_a_busy_dmem_dpc_source() {
    let dmem_busy = fn64_runtime::DPC_STATUS_XBUS_DMEM_DMA | fn64_runtime::DPC_STATUS_DMA_BUSY;
...
fn chunked_hle_observes_sig0_between_commits_and_consumes_resume_once() {
    ...
    impl RenderBackend for ChunkedBackend {
        ...
        fn process_task_chunk(
            ...
            Ok(match step {
                fn64_render::RenderTaskStep::Start => fn64_render::RenderTaskChunkStatus::Continue(
...
fn direct_imem_chunk_yield_public_resume_completes_with_resumed_generation_owner() {
    ...
    assert_eq!(observations[0].key.admission_generation, initial_generation);
    ...
    crate::drain_guest_task_observations(&mut observations);
    assert_eq!(observations.len(), 1);
    assert_eq!(observations[0].key.admission_generation, resumed_generation);
    assert_eq!(
...
fn os_sp_task_start_go_routes_gfx_tasks_through_the_registered_render_backend() {
    ...
    rdram[HEADER_OFF..HEADER_OFF + 4].copy_from_slice(&fn64_runtime::M_GFXTASK.to_ne_bytes());
    rdram[HEADER_OFF + 0x30..HEADER_OFF + 0x34].copy_from_slice(&(DL_ADDR as u32).to_ne_bytes());
