...
fn diagnostic_graphics_skip_advances_the_sp_then_dp_scheduler() {
    let full_sync = diagnostic_graphics_dp_full_sync(GraphicsTaskExecutionPolicy::DiagnosticSkip)
...
fn whole_audio_capture_matches_begin_state_and_mutates_only_its_owner() {
    ...
    let captured = unsafe { capture_audio_whole_task_input(rdram.as_mut_ptr(), task_addr, header) };
...
fn verified_audio_patches_use_logical_guest_byte_order() {
    let patches = fn64_audio::hle_outcome::CanonicalRdramPatches::new(vec![
        fn64_audio::hle_outcome::RdramPatch::new(1, vec![0x11, 0x22, 0x33, 0x44, 0x55]).unwrap(),
    ])
    .unwrap();
...
fn every_renderer_entry_traps_when_no_backend_is_registered() {
    ...
    assert!(
        panic_message(panic.as_ref()).contains("renderer_gate_test: no render backend registered")
    );
}
...
fn unsupported_backend_ucode_records_typed_event_before_loud_failure() {
    ...
    assert!(panic_message(panic.as_ref()).contains("unsupported ucode at rdram offset 0x00123450"));
    assert_eq!(
    ...
    impl RenderBackend for CaptureBackend {
        ...
        fn render_target_diagnostic(
            ...
            Ok(fn64_render::RenderTargetDiagnostic {
                ...
                workload_resolution_scale: fn64_render::RenderResolutionScale::try_new(2.0, 2.0)
                    .unwrap(),
                resolution_scale: fn64_render::RenderResolutionScale::try_new(2.0, 2.0).unwrap(),
