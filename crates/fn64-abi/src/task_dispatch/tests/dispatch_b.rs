...
fn direct_imem_hle_needs_lle_replays_untouched_pc_zero_entry_through_public_task_path() {
    ...
    let RspInterpreterStateEvidenceSnapshot::Exact(state) = evidence.rsp_interpreter_state else {
...
fn pinned_family_authority_fills_absent_backend_identity_and_rejects_conflict() {
    ...
    let panic =
        std::panic::catch_unwind(|| identify_microcode_pair(&imem, data, Some(UcodeId::F3dzex2)))
            .unwrap_err();
...
fn graphics_lle_accuracy_policy_forwards_raw_dpc_without_hle_dispatch() {
    ...
    with_host(|host| {
        ...
        for (offset, word) in [
            ...
            memory
                .write_word(
                    fn64_runtime::RspMemAddr::from_parts(fn64_runtime::RspMemoryBank::Imem, offset),
                    word,
                )
                .unwrap();
        }
