...
fn matches_view_agrees_with_the_snapshot_comparison_at_every_offset() {
    ...
    for &(start, end) in &[(0x101u32, 0x117u32), (0x100, 0x120), (0x102, 0x106)] {
        ...
        let mut state = CanonicalExecutableMutationStateV1::new(&[(start, end)]);
        {
            ...
            state
                .seal_with(|physical| view.read_u8(fn64_runtime::RdramAddr::from_offset(physical)));
        }
        {
            let view = fn64_runtime::RdramView::from_storage(&storage);
            assert!(
    ...
    for &layout in &[
        ...
        for _ in 0..32 {
            ...
            for (fast_range, slow_range) in fast.watched.iter().zip(&slow.watched) {
                ...
                assert_eq!(fast_range.expected_page_tree, slow_range.expected_page_tree);
            }
...
fn renderer_transaction_attributes_exact_changed_executable_bytes() {
    ...
    let previous = fn64_cpu_runtime::set_write_observer(Some(record_executable_and_renderer_write));
...
fn suspended_host_transaction_orders_same_byte_device_write_before_resume_suffix() {
    ...
    let resolver = CatalogResolverInstallV1::new(
        ...
        .unwrap(),
        HostFunctionCatalogV1::new(vec![(ORDERED_WRITER_HOST.get(), ordered_writer_host)]).unwrap(),
...
fn page_tree_root_is_independent_of_incremental_history() {
    ...
    let assert_trees_match =
        |state: &CanonicalExecutableMutationStateV1, image0: &[u8], image1: &[u8], what: &str| {
...
fn the_v3_merkle_root_differs_from_both_the_v1_flat_and_v2_page_digests() {
    ...
    let v2: [u8; 32] = {
        ...
        watched_root_digest_v2(std::iter::once((
            ranges[0].0,
            ranges[0].1,
            leaves.as_slice(),
        )))
...
fn the_v3_tree_binds_its_structure() {
    ...
    assert_ne!(
        leaf, promoted,
        "leaf and node messages must be tagged apart"
    );
    assert_ne!(leaf, h1);
    ...
    let _ = (
        r5, r_other, i1, other, other_end, swapped, leaf, promoted, h1, r4,
    );
}
...
fn an_empty_dirty_set_reports_a_match_without_reading() {
    ...
    {
        let view = fn64_runtime::RdramView::from_storage(&storage);
        state.seal_with(|physical| view.read_u8(fn64_runtime::RdramAddr::from_offset(physical)));
    }
...
fn dirty_span_normalization_is_ascending_disjoint_and_clipped() {
    ...
    assert_eq!(
        clip(&merged, 0x200, 0x340),
        vec![(0x200, 0x280), (0x300, 0x340)]
    );
