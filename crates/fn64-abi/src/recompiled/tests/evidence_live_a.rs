...
fn function_destination_history_binds_artifact_function_cycle_and_schema() {
    ...
    fn64_cpu_runtime::notify_function_entry(TranslatedFunctionIdentity::new(0x8000_1000, "entry"));
    with_executor(|executor| executor.advance_time(41));
    fn64_cpu_runtime::notify_function_entry(TranslatedFunctionIdentity::new(0x8000_2000, "callee"));
...
fn executable_write_preflight_test_scope_restores_on_unwind() {
    let _outer = scoped_test_executable_write_preflight_state(vec![(0x20, 0x40)], vec![(0x24, 4)]);

    let panic = std::panic::catch_unwind(|| {
        let _inner =
            scoped_test_executable_write_preflight_state(vec![(0x100, 0x180)], vec![(0x120, 8)]);
        assert_eq!(
    ...
    generation_catalog
        .register(
            PrecompiledGeneration::new(
                ...
                vec![
                    PrecompiledShard::new(watched_bank, host_pc, GuestPc::new(host_pc.get() + 4))
                        .unwrap(),
                ],
            )
            .unwrap(),
