...
fn captured_function_lane_derives_entry_and_restores_cpu_before_dispatch() {
    ...
    boot.cp0.registers[13] = u64::from(fn64_cpu_runtime::CpuInterruptLine::TIMER.cause_bit());
...
#[test]
fn created_osthread_applies_saved_sr_restore_without_discarding_cu_state() {
    let inherited = 0xA5A5_5A5A | STATUS_FR;

    let ctx = new_osthread_context(Some(inherited));

    assert_eq!(ctx.cop0_status, 0xA000_FF01);
    assert_eq!(ctx.os_interrupt_mask(), 0x003f_ff01);
    assert_eq!(ctx.read_fcr(31), INITIAL_FPCSR);
}
...
fn typed_interrupt_masks_return_each_contexts_own_previous_value() {
    const THREAD_A: ThreadId = 0x1a11;
    const THREAD_B: ThreadId = 0x1b11;
    let observed_a = Rc::new(RefCell::new(Vec::new()));
    let observed_b = Rc::new(RefCell::new(Vec::new()));
    let body_a = Rc::clone(&observed_a);
    let body_b = Rc::clone(&observed_b);
    let initial = fn64_runtime::SavedRcpInterruptMask::from_bits(0).unwrap();
    ...
    with_executor(|exec| {
        exec.create_thread_with_saved_rcp_interrupt_mask(
            ...
            move |yielder, _| {
                ...
                let mut mem = Rdram::new(&mut bytes);
                let mut ctx = RsContext::new();
                ctx.set_r(4, 0x0010_0401);
                os_set_int_mask(&mut ctx, &mut mem);
                body_a.borrow_mut().push(ctx.r_u32(2));
                let _ = yielder.suspend(fn64_runtime::Yield::PauseSelf);
                ctx.set_r(4, 0x0004_0401);
                os_set_int_mask(&mut ctx, &mut mem);
                body_a.borrow_mut().push(ctx.r_u32(2));
            },
        );
        exec.create_thread_with_saved_rcp_interrupt_mask(THREAD_B, 5, initial, move |_, _| {
            let mut bytes = [0; 4];
            let mut mem = Rdram::new(&mut bytes);
            let mut ctx = RsContext::new();
            ctx.set_r(4, 0x0008_0401);
            os_set_int_mask(&mut ctx, &mut mem);
            body_b.borrow_mut().push(ctx.r_u32(2));
        });
        exec.start_thread(THREAD_A);
        exec.start_thread(THREAD_B);
    });
    ...
    assert!(crate::run_one_step());
    assert!(crate::run_one_step());
    assert_eq!(&*observed_a.borrow(), &[0, 0x0010_0401]);
    assert_eq!(&*observed_b.borrow(), &[0]);
}
...
fn typed_raw_vi_registers_drive_half_line_timing_and_shared_mi() {
    crate::test_support::install_complete_render_backend(fn64_runtime::rdram::DEFAULT_RDRAM_SIZE);
    let previous = fn64_cpu_runtime::set_mmio_hooks(Some(read_raw_mmio), Some(write_raw_mmio));
    let mut bytes = [0; 4];
    let mut mem = Rdram::new(&mut bytes);
