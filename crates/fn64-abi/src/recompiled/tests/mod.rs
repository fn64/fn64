...
use fn64_cpu_runtime::{
    ...
    CodeBank, CodeCatalog, CpuFaultKind, GeneratedBankRunner, GenerationId, PhysicalCodeBank,
    PrecompiledGeneration, PrecompiledGenerationBackingV1, PrecompiledShard, Sha256Digest,
...
fn bootstrap_test_install(expected_word: u32) -> CatalogGenerationInstallV1 {
    ...
    let generations =
        BackedPrecompiledGenerationCatalogV1::new(PrecompiledGenerationCatalog::new(), Vec::new())
            .unwrap();
...
fn sp_test_trace(direction: fn64_runtime::SpDmaDirection) -> Vec<fn64_runtime::DeviceTraceEvent> {
...
fn install_public_si_runtime_state_test_owner() -> fn64_runtime::SiDmaRequest {
    ...
    let state =
        CanonicalExecutableMutationStateV1::from_bootstrap(&bootstrap_evidence, &validated.storage);
...
fn observe_captured_function_boot(ctx: &mut RsContext, _mem: &mut Rdram<'_>) {
    FUNCTION_BOOT_EXPECTED.with(|expected| {
        ...
        let expected = expected
            .as_ref()
            .expect("captured boot expectation installed");
...
fn install_evidence_block_lane(budget: u32, reverse_regions: bool, alternate_builders: bool) {
    ...
    let mut program = BlockProgram::new();
    let mut first_region = ExecutableRegion::new(first_start, GuestPc::new(first_start.get() + 4));
...
fn rewrite_interpreter_runner(
    ...
    match run.exit {
        BlockExit::ResolveTransfer { target_pc, .. } if ctx.is_thread_return(target_pc.get()) => {
...
fn dma_rewrite_builder(
    ...
    Ok((
        CodeBank::new(DMA_NEW_BANK, DMA_ENTRY, vec![1, 1]).map_err(|error| error.to_string())?,
...
pub(in crate::recompiled) use c_adapter::{
    change_bev_shim, change_fr_shim, no_op_fpr_shim, write_f5_word_shim,
};
