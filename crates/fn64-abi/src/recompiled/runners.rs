...
macro_rules! c_adapters {
    ($(($recompiled:ident, $shim:ident)),+ $(,)?) => {
        ...
        /// Snake-case names of the host adapters generated by THIS macro. The
        /// complete authoritative surface is [`ADAPTER_NAMES`], which appends
        /// the hand-written adapters (`os_initialize`, CPU-register/thread
        /// adapters) that live outside the table.
        const MACRO_ADAPTER_NAMES: &[&str] = &[$(stringify!($recompiled)),+];
    };
}
...
/// Host adapters that are hand-written `pub fn`s in this file rather than
/// generated by `c_adapters!` — they need custom bodies (CPU status/cause
/// registers, thread init, the framebuffer query) instead of a straight
/// `call_c` bridge. Keep this in sync with the `pub fn <name>(ctx: &mut
/// RsContext, mem: &mut Rdram)` definitions above; the
/// `adapter_names_are_all_defined` test fails loudly if a name here has no
/// definition. Together with `MACRO_ADAPTER_NAMES` this is the full surface.
pub(in crate::recompiled) const HAND_ADAPTER_NAMES: &[&str] = &[
    "os_initialize",
    "os_create_thread",
    "os_get_sr",
    "os_set_sr",
...
/// The complete, authoritative set of host-adapter names exposed as `pub fn`
/// in `fn64_abi::recompiled` — macro-generated plus hand-written. The rs-lane
/// `host_lookup.rs` generator (fn64-cpu-runtime-codegen) joins this against a
/// title's symbol dump so the vram->adapter table can never drift from the
/// adapters that actually exist.
pub fn adapter_names() -> Vec<&'static str> {
    MACRO_ADAPTER_NAMES
        .iter()
        .chain(HAND_ADAPTER_NAMES)
        .copied()
        .collect()
}
