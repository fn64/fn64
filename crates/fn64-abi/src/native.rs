...
pub fn set_entry_lookup(lookup: NativeLookup, rdram_len: usize) {
    assert!(rdram_len > 0, "native RDRAM length must be nonzero");
    with_host(|host| {
        host.native_lookup = Some(lookup);
        host.native_rdram_len = rdram_len;
    });
}

fn pause_active_native_thread() {
    super::suspend_active_coroutine(fn64_runtime::Yield::PauseSelf);
}

/// Create and start thread 0 with a typed native entrypoint on fn64's existing
...
pub unsafe fn boot_thread0(
    ...
    fn64_recomp_native::set_host_pause(Some(pause_active_native_thread));
...
/// `osCreateThread_recomp` and `native::boot_thread0`.
pub(super) unsafe fn run_registered_entry(
    ...
    let registered = with_host(|host| {
        host.native_lookup
            .map(|lookup| (lookup, host.native_rdram_len))
    });
    let Some((lookup, rdram_len)) = registered else {
        return false;
    };
    assert!(rdram_len > 0, "native entry lookup has no RDRAM length");
...
fn c_from_native(ctx: &NativeContext) -> CContext {
    let r = ctx.gprs();
...
fn copy_c_back(c: &CContext, ctx: &mut NativeContext) {
    ctx.set_gprs([
...
fn call_c(ctx: &mut NativeContext, mem: &mut Rdram<'_>, name: &'static str, shim: CShim) {
    if std::env::var_os("FN64_NATIVE_SHIM_TRACE").is_some() {
        eprintln!("[fn64-native-shim] {name}");
    }
    let mut c = c_from_native(ctx);
...
macro_rules! c_adapters {
    ($(($native:ident, $shim:ident)),+ $(,)?) => {$ (
        pub fn $native(ctx: &mut NativeContext, mem: &mut Rdram<'_>) {
...
pub fn os_get_sr(ctx: &mut NativeContext, _mem: &mut Rdram<'_>) {
    ctx.set_r(2, ctx.cop0_status as u64);
}

/// `__osSetSR`: replace this OSThread's typed COP0 Status register.
pub fn os_set_sr(ctx: &mut NativeContext, _mem: &mut Rdram<'_>) {
...
pub fn os_get_cause(ctx: &mut NativeContext, _mem: &mut Rdram<'_>) {
    ctx.set_r(2, ctx.cop0_cause as u64);
}

/// `osGetIntMask`: return the same process-local interrupt mask maintained by
/// `osSetIntMask_recomp`.
pub fn os_get_int_mask(ctx: &mut NativeContext, _mem: &mut Rdram<'_>) {
...
mod tests {
    ...
    fn c_adapter_round_trips_all_gprs_and_forces_zero() {
        ...
        for i in 1..32 {
            native.set_r(i, 0xA000_0000_0000_0000 | i as u64);
        }
        let mut c = c_from_native(&native);
        c.r0 = u64::MAX;
        c.r2 = 0x1234;
        copy_c_back(&c, &mut native);
        assert_eq!(native.r(0), 0);
        assert_eq!(native.r(2), 0x1234);
        assert_eq!(native.r(31), 0xA000_0000_0000_001F);
    }
