...
//! In this crate that left the audio-priority pair, which is now installed by
//! the host through `task_dispatch::set_audio_priority_join_budget_ms` and
//! `set_audio_priority_vi_presentation` rather than read from the
//! environment at all.
//!
//! What remains are the `diagnostic`-class sinks: census counters, task and
//! stream dumps, timing probes, trace switches. They are set by a gate
//! harness or by hand for one investigation and are read nowhere in a shipped
//! configuration.
//!
