    ...
    fn si_device_busy_is_unique_and_invariant_sensitive() {
        ...
        for index in 2..8 {
            ...
            broken[index] ^= if index == 7 { 2 } else { 1 };
            assert!(matches!(
        ...
        for symbol in symbols {
            ...
            assert_eq!(
                ...
                if symbol == HostBindingSymbol::OsCreateThread {
                    HostSpawnedStatusEffect::GeneratedSavedSrPostEretClearsBev
                } else {
...
mod external_reference_fallback {
    ...
    fn world_tour_external_table_reaches_15_of_15_with_agreement_and_validation() {
        if let Ok(path) = std::env::var("WT_ROM") {
            ...
            let rom = crate::normalize(&source).expect("normalizing WT_ROM");
            let mut facts = crate::FactDb::new();
            let discovery = crate::banks::discover_boot_bank(&rom, &mut facts);
            assert!(matches!(discovery, crate::banks::BootBankDiscovery::Proven { .. }));
            let va_start = facts
                .proven_rom_mappings()
                .into_iter()
                .find_map(|fact| match fact {
                    crate::Fact::RomMapping { bank, va_start, .. }
                        if bank == crate::banks::BOOT_BANK => Some(*va_start),
                    _ => None,
                })
                .expect("WT boot-bank mapping");
            let start = crate::banks::BOOT_COPY_ROM_START as usize;
            let end = start + crate::banks::BOOT_COPY_SIZE as usize;
            let words = rom.bytes[start..end]
                .chunks_exact(4)
                .map(|bytes| u32::from_be_bytes(bytes.try_into().unwrap()))
                .collect::<Vec<_>>();
            let outcomes: BTreeMap<_, _> = probe_wm_block_runtime_host_bindings(&words, va_start)
                .into_iter()
                .collect();
            eprintln!("WT host-binding outcomes: {outcomes:#x?}");
            for (symbol, expected) in [
                (HostBindingSymbol::OsSpTaskLoad, 0x8001_29fc),
                (HostBindingSymbol::OsSpTaskStartGo, 0x8001_2b8c),
                (HostBindingSymbol::OsSpTaskYield, 0x8001_2c80),
                (HostBindingSymbol::OsSpTaskYielded, 0x8001_2760),
                (HostBindingSymbol::OsStartThread, 0x8001_16b0),
                (HostBindingSymbol::OsSendMesg, 0x8001_1c20),
                (HostBindingSymbol::OsRecvMesg, 0x8001_1f00),
                (HostBindingSymbol::OsSetEventMesg, 0x8001_25e0),
                (HostBindingSymbol::OsSiDeviceBusy, 0x8002_5250),
                (HostBindingSymbol::OsSetTimer, 0x8002_3820),
            ] {
                assert_eq!(
                    outcomes.get(&symbol),
                    Some(&HostBindingProbeOutcome::Resolved { vram: expected }),
                    "WT resolution for {symbol:?}: {:?}",
                    outcomes.get(&symbol),
                );
            }
            let discovered = discover_wm_block_runtime_host_bindings(&words, va_start)
                .expect("WT recognizer-only 15/15 discovery");
            assert_eq!(discovered.len(), WM_BLOCK_RUNTIME_HOST_SYMBOLS.len());
            return;
        }
    ...
    fn empty_external_table_keeps_pure_recognizer_provenance() {
        ...
        for binding in &resolved {
            ...
            assert_eq!(binding.vram, addresses[&binding.symbol], "{:?}", binding.symbol);
        }
    }
}
