//! Source-generation half of `fn64-cpu-runtime`.
//!
//! Generated runners link only `fn64-cpu-runtime`. Keeping this crate on the
...
pub use fn64_cpu_runtime::{BankId, BankWordKind};
...
pub fn instruction_has_delay_slot(word: u32) -> bool {
    fn64_cpu_runtime::decode(word).has_delay_slot()
}
...
impl Recompiler for RsRecompiler {
    fn recompile(&self, cfg: &RecompConfig) -> Result<RecompOutput, RecompError> {
        let rom = std::fs::read(&cfg.rom_file_path).map_err(|error| RecompError::Launch {
            binary: "fn64-cpu-runtime".to_string(),
            reason: format!(
        ...
        let symbols = SymbolTable::from_section_entries(cfg.sections.iter().enumerate().flat_map(
            |(section_index, section)| {
                let (stubbed, ignored) = (&stubbed, &ignored);
                section.functions.iter().filter_map(move |function| {
                    (!stubbed.contains(function.name.as_str())
                        && !ignored.contains(function.name.as_str()))
                    .then(|| (section_index, function.name.clone(), function.vram))
                })
            },
        ));
        let mut body = String::new();
        body.push_str(
            "// Generated by fn64-cpu-runtime. Typed Rust, no unsafe, no pointer casts.\n",
        );
        body.push_str(
        ...
        body.push_str("pub const FN64_FUNCTION_ENTRY_OBSERVATION_SCHEMA: fn64_cpu_runtime::FunctionEntryObservationSchema = fn64_cpu_runtime::FUNCTION_ENTRY_OBSERVATION_SCHEMA;\n");
        body.push_str("#[allow(unused_imports)]\n");
        body.push_str("use fn64_cpu_runtime::{call_host_or_recompiled, pause_self, resolve_host_function, RecompContext, RecompFunc, Rdram, round_ties_even_f32, round_ties_even_f64};\n\n");
    ...
    fn recompile_rsp(&self, _cfg: &RspConfig) -> Result<RecompOutput, RecompError> {
        Err(RecompError::InvalidConfig(
            "fn64-cpu-runtime is a CPU (VR4300) recompiler; RSP microcode is out of scope \
