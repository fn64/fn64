...
use fn64_audio::characterize::{verify_compact_request, CharacterizationRequest};

fn main() {
    if let Err(error) = run() {
        eprintln!("compact audio verification failed: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let report = verify_compact_request(request)?;
    println!(
        "{}",
        serde_json::to_string(&report)
            .map_err(|error| format!("serialize compact verification report: {error}"))?
    );
    Ok(())
}
