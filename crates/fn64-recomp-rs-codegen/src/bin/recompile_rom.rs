//! Whole-ROM driver for `fn64-recomp-rs`.
//!
...
//!
//! This is the link gate for the "prove `fn64-recomp-rs` can recompile the
...
const GENERATED_USE: &str = "use fn64_recomp_rs::{call_host_or_recompiled, pause_self, resolve_host_function, RecompContext, RecompFunc, Rdram, round_ties_even_f32, round_ties_even_f64};\n\n";
...
fn run(cfg: &RecompConfig, rom: &[u8], force_recompile: &HashSet<String>) -> Report {
    ...
    module.push_str("// Generated by fn64-recomp-rs whole-ROM driver (recompile_rom).\n");
    module.push_str(
    ...
    for (index, bodies) in part_bodies.into_iter().enumerate() {
        ...
        part.push_str("// Generated by fn64-recomp-rs whole-ROM driver (recompile_rom).\n");
...
fn render_generated_manifest() -> String {
    ...
    format!(
        "# Generated by fn64-recomp-rs whole-ROM driver (recompile_rom).\n\
         ...
         publish = false\n\n\
         [dependencies]\n\
         fn64-recomp-rs = {{ path = {quoted_path} }}\n"
    )
}

fn render_generated_lib(symbols: &SymbolTable, sections: &[Section]) -> String {
    let mut root = String::new();
    root.push_str("// Generated by fn64-recomp-rs whole-ROM driver (recompile_rom).\n");
    root.push_str(
...
impl Report {
    ...
    fn render_markdown(&self, cfg: &RecompConfig) -> String {
        ...
        let c = self.counts();
        let mut s = String::new();
        s.push_str("# fn64-recomp-rs whole-ROM gap report (OoT NTSC 1.0)\n\n");
        s.push_str(&format!(
        ...
        s.push_str(&format!(
            "The emitted module contains **{}** `lookup(addr)(ctx, mem)` call sites \
             ...
             `fn64_recomp_rs::resolve_host_function`, so host-owned vrams override recompiled \
...
mod tests {
    ...
    fn runtime_trap_is_host_bound_for_direct_and_indirect_dispatch() {
        ...
        assert!(manifest.contains("fn64-recomp-rs = { path = "));
        assert!(manifest.contains("/crates/fn64-recomp-rs\" }"));
